...
typedef struct {
    ...
    uint32_t Page;      /*!< Initial Flash page to erase when page erase is disabled
                             This parameter must be a value between 0 and (max number of pages in the
                           bank - 1)      (eg : 255 for 1MB dual bank) */
    uint32_t NbPages;   /*!< Number of pages to be erased.
                             This parameter must be a value between 1 and (max number of pages in the
                           bank - value of initial page)*/
...
typedef struct {
    ...
    uint32_t WRPStartOffset; /*!< Write protection start offset (used for OPTIONBYTE_WRP).
                                ...
                                in the bank - 1) (eg : 25 for 1MB dual bank) */
    ...
    uint32_t PCROPConfig;    /*!< Configuration of the PCROP (used for OPTIONBYTE_PCROP).
                                  This parameter must be a combination of @ref FLASH_Banks (except
    ...
    uint32_t PCROPEndAddr;   /*!< PCROP End address (used for OPTIONBYTE_PCROP).
                                  This parameter must be a value between PCROP Start address and end
                                of bank */
...
typedef struct {
    ...
    __IO uint32_t
        Address; /* Internal variable to save address selected for program in IT context */
    __IO uint32_t
        Bank; /* Internal variable to save current bank selected during erase in IT context */
    __IO uint32_t
        Page; /* Internal variable to define the current page which is erasing in IT context */
    __IO uint32_t
        NbPagesToErase; /* Internal variable to save the remaining pages to erase in IT context */
    __IO FLASH_CacheTypeDef
        CacheToReactivate; /* Internal variable to indicate which caches should be reactivated */
...
#define OB_BOR_LEVEL_0                                                                             \
    ((uint32_t)FLASH_OPTR_BOR_LEV_0) /*!< Reset level threshold is around 1.7V                     \
                                      */
#define OB_BOR_LEVEL_1                                                                             \
    ((uint32_t)FLASH_OPTR_BOR_LEV_1) /*!< Reset level threshold is around 2.0V                     \
                                      */
#define OB_BOR_LEVEL_2                                                                             \
    ((uint32_t)FLASH_OPTR_BOR_LEV_2) /*!< Reset level threshold is around 2.2V                     \
                                      */
#define OB_BOR_LEVEL_3                                                                             \
    ((uint32_t)FLASH_OPTR_BOR_LEV_3) /*!< Reset level threshold is around 2.5V                     \
                                      */
#define OB_BOR_LEVEL_4                                                                             \
    ((uint32_t)FLASH_OPTR_BOR_LEV_4) /*!< Reset level threshold is around 2.8V                     \
                                      */
/**
 * @}
 */
...
#define OB_STOP_NORST                                                                              \
    ((uint32_t)FLASH_OPTR_nRST_STOP) /*!< No reset generated when entering the stop mode */
/**
 * @}
 */
...
#define OB_STANDBY_NORST                                                                           \
    ((uint32_t)FLASH_OPTR_nRST_STDBY) /*!< No reset generated when entering the standby mode */
/**
 * @}
 */
...
#define OB_IWDG_STOP_RUN                                                                           \
    ((uint32_t)FLASH_OPTR_IWDG_STOP) /*!< Independent watchdog counter is running in Stop mode */
/**
 * @}
 */
...
#define OB_BOOT1_SYSTEM                                                                            \
    ((uint32_t)FLASH_OPTR_nBOOT1) /*!< System memory is selected as boot space (if BOOT0=1) */
/**
 * @}
 */
...
#define OB_SRAM2_RST_NOT_ERASE                                                                     \
    ((uint32_t)FLASH_OPTR_SRAM2_RST) /*!< SRAM2 is not erased when a system reset occurs */
/**
 * @}
 */
...
#define FLASH_FLAG_SR_ERRORS                                                                       \
    (FLASH_FLAG_OPERR | FLASH_FLAG_PROGERR | FLASH_FLAG_WRPERR | FLASH_FLAG_PGAERR |               \
     ...
     FLASH_FLAG_RDERR | FLASH_FLAG_OPTVERR | FLASH_FLAG_PEMPTY)
#else
#define FLASH_FLAG_SR_ERRORS                                                                       \
    (FLASH_FLAG_OPERR | FLASH_FLAG_PROGERR | FLASH_FLAG_WRPERR | FLASH_FLAG_PGAERR |               \
     FLASH_FLAG_SIZERR | FLASH_FLAG_PGSERR | FLASH_FLAG_MISERR | FLASH_FLAG_FASTERR |              \
...
#define FLASH_FLAG_ALL_ERRORS                                                                      \
    (FLASH_FLAG_OPERR | FLASH_FLAG_PROGERR | FLASH_FLAG_WRPERR | FLASH_FLAG_PGAERR |               \
     FLASH_FLAG_SIZERR | FLASH_FLAG_PGSERR | FLASH_FLAG_MISERR | FLASH_FLAG_FASTERR |              \
     FLASH_FLAG_RDERR | FLASH_FLAG_OPTVERR | FLASH_FLAG_ECCR_ERRORS)
/**
 * @}
 */
 ...
 *     @arg FLASH_FLAG_PEMPTY : FLASH Boot from not programmed flash (apply only for
     ...
     ((((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) & (0x0FFFU)) == 0x400U)                          \
          ...
          : ((((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) & (0x0FFFU)) == 0x200U)                   \
                 ...
                 : ((((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) & (0x0FFFU)) == 0x100U)            \
                        ...
                        : ((((*((uint16_t *)FLASH_SIZE_DATA_REGISTER)) & (0x0FFFU)) == 0x80U)      \
...
#define IS_OPTIONBYTE(VALUE)                                                                       \
    (((VALUE) <= (OPTIONBYTE_WRP | OPTIONBYTE_RDP | OPTIONBYTE_USER | OPTIONBYTE_PCROP)))
...
#define IS_OB_USER_TYPE(TYPE)                                                                      \
    (((TYPE) <= (uint32_t)0x7E7FU) && ((TYPE) != 0U) && (((TYPE)&0x0180U) == 0U))
#endif
...
#define IS_OB_USER_SRAM2_PARITY(VALUE)                                                             \
    (((VALUE) == OB_SRAM2_PARITY_ENABLE) || ((VALUE) == OB_SRAM2_PARITY_DISABLE))
