...
#define __HAL_SYSCFG_REMAPMEMORY_OCTOSPI1()                                                        \
    MODIFY_REG(SYSCFG->MEMRMP, SYSCFG_MEMRMP_MEM_MODE, (SYSCFG_MEMRMP_MEM_MODE_2))
 ...
 *         Enable and lock the connection of Flash ECC error connection to TIM1/8/15/16/17 Break
 * input.
 ...
 *         Enable and lock the connection of Cortex-M4 LOCKUP (Hardfault) output to TIM1/8/15/16/17
 * Break input.
 * @note   The selected configuration is locked and can be unlocked only by system reset.
 */
 ...
 *         Enable and lock the PVD connection to Timer1/8/15/16/17 Break input, as well as the PVDE
 * and PLS[2:0] in the PWR_CR2 register.
 * @note   The selected configuration is locked and can be unlocked only by system reset.
 */
