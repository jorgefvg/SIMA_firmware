...
__STATIC_INLINE void LL_DMA_EnableChannel(DMA_TypeDef * DMAx, uint32_t Channel) {
    ...
    SET_BIT(((DMA_Channel_TypeDef *)(dma_base_addr + CHANNEL_OFFSET_TAB[Channel]))->CCR,
            DMA_CCR_EN);
}
...
__STATIC_INLINE void LL_DMA_DisableChannel(DMA_TypeDef * DMAx, uint32_t Channel) {
    uint32_t dma_base_addr = (uint32_t)DMAx;
    CLEAR_BIT(((DMA_Channel_TypeDef *)(dma_base_addr + CHANNEL_OFFSET_TAB[Channel]))->CCR,
              DMA_CCR_EN);
}
...
__STATIC_INLINE void LL_DMA_ConfigAddresses(DMA_TypeDef * DMAx, uint32_t Channel,
    ...
    if (Direction == LL_DMA_DIRECTION_MEMORY_TO_PERIPH) {
        WRITE_REG(((DMA_Channel_TypeDef *)(dma_base_addr + CHANNEL_OFFSET_TAB[Channel]))->CMAR,
                  SrcAddress);
        WRITE_REG(((DMA_Channel_TypeDef *)(dma_base_addr + CHANNEL_OFFSET_TAB[Channel]))->CPAR,
                  DstAddress);
    }
...
__STATIC_INLINE void LL_DMA_SetPeriphRequest(DMA_TypeDef * DMAx, uint32_t Channel,
    ...
    MODIFY_REG((DMAMUX1_Channel0 + Channel + dmamux_ccr_offset)->CCR, DMAMUX_CxCR_DMAREQ_ID,
               Request);
}
 ...
 * @note   Please refer to Reference Manual to get the available mapping of Request value link to
 * Channel Selection.
...
__STATIC_INLINE void LL_DMA_SetPeriphRequest(DMA_TypeDef * DMAx, uint32_t Channel,
    ...
    MODIFY_REG(((DMA_Request_TypeDef *)((uint32_t)((uint32_t)DMAx + DMA_CSELR_OFFSET)))->CSELR,
               DMA_CSELR_C1S << (((Channel)*4U) & 0x1FU),
               PeriphRequest << DMA_POSITION_CSELR_CXS(Channel));
}
...
__STATIC_INLINE uint32_t LL_DMA_GetPeriphRequest(DMA_TypeDef * DMAx, uint32_t Channel) {
    return (
        READ_BIT(((DMA_Request_TypeDef *)((uint32_t)((uint32_t)DMAx + DMA_CSELR_OFFSET)))->CSELR,
