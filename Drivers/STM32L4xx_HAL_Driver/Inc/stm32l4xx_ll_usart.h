...
#define LL_USART_BITORDER_MSBFIRST                                                                 \
    USART_CR2_MSBFIRST /*!< data is transmitted/received with the MSB first, following the start   \
                          bit */
/**
 * @}
 */
...
#define LL_USART_AUTOBAUD_DETECT_ON_FALLINGEDGE                                                    \
    USART_CR2_ABRMODE_0 /*!< Falling edge to falling edge measurement. Received frame must start   \
                           with a single bit = 1 -> Frame = Start10xxxxxx */
...
#define LL_USART_HWCONTROL_CTS                                                                     \
    USART_CR3_CTSE /*!< CTS mode enabled, data is only transmitted when the nCTS input is asserted \
                      (tied to 0) */
...
#define __LL_USART_DIV_SAMPLING8(__PERIPHCLK__, __PRESCALER__, __BAUDRATE__)                       \
    (((((__PERIPHCLK__) / (USART_PRESCALER_TAB[(__PRESCALER__)])) * 2U) + ((__BAUDRATE__) / 2U)) / \
     (__BAUDRATE__))
#else
