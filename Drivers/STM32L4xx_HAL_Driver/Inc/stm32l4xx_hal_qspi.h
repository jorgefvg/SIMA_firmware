 ...
 */
typedef struct {
    uint32_t ClockPrescaler; /* Specifies the prescaler factor for generating clock based on the AHB
                                clock. This parameter can be a number between 0 and 255 */
    uint32_t FifoThreshold;  /* Specifies the threshold number of bytes in the FIFO (used only in
                                indirect mode)  This parameter can be a value between 1 and 16 */
    uint32_t SampleShifting; /* Specifies the Sample Shift. The data is sampled 1/2 clock cycle
                                delay later to take in account external signal delays. (It should be
                                QSPI_SAMPLE_SHIFTING_NONE in DDR mode) This parameter can be a value
                                of @ref QSPI_SampleShifting */
    uint32_t
        FlashSize; /* Specifies the Flash Size. FlashSize+1 is effectively the number of address
                      bits required to address the flash memory. The flash capacity can be up to 4GB
                      (addressed using 32 bits) in indirect mode, but the addressable space in
                      memory-mapped mode is limited to 256MB
                      This parameter can be a number between 0 and 31 */
    uint32_t ChipSelectHighTime; /* Specifies the Chip Select High Time. ChipSelectHighTime+1
                                    defines the minimum number of clock cycles which the chip select
                                    must remain high between commands. This parameter can be a value
                                    of @ref QSPI_ChipSelectHighTime */
    uint32_t ClockMode; /* Specifies the Clock Mode. It indicates the level that clock takes between
                           commands. This parameter can be a value of @ref QSPI_ClockMode */
...
typedef enum {
    ...
    HAL_QSPI_STATE_BUSY_INDIRECT_TX =
        0x12U, /*!< Peripheral in indirect mode with transmission ongoing */
    HAL_QSPI_STATE_BUSY_INDIRECT_RX =
        0x22U, /*!< Peripheral in indirect mode with reception ongoing    */
...
typedef struct {
    ...
    uint32_t AlternateBytes;     /* Specifies the Alternate Bytes to be sent (Size from 1 to 4 bytes
                                    according AlternateBytesSize)     This parameter can be a value
    ...
    uint32_t NbData;  /* Specifies the number of data to transfer. (This is the number of bytes)
                         This parameter can be any value between 0 and 0xFFFFFFFF (0 means undefined
                         length  until end of memory)*/
    uint32_t DdrMode; /* Specifies the double data rate mode for address, alternate byte and data
                         phase This parameter can be a value of @ref QSPI_DdrMode */
    uint32_t
 ...
 */
typedef struct {
    uint32_t Match; /* Specifies the value to be compared with the masked status register to get a
                       match. This parameter can be any value between 0 and 0xFFFFFFFF */
    uint32_t Mask;  /* Specifies the mask to be applied to the status bytes received.
                       This parameter can be any value between 0 and 0xFFFFFFFF */
    uint32_t Interval; /* Specifies the number of clock cycles between two read during automatic
                          polling phases. This parameter can be any value between 0 and 0xFFFF */
 ...
 */
typedef struct {
    uint32_t TimeOutPeriod; /* Specifies the number of clock to wait when the FIFO is full before to
                               release the chip select. This parameter can be any value between 0
                               and 0xFFFF */
    uint32_t TimeOutActivation; /* Specifies if the timeout counter is enabled to release the chip
                                   select. This parameter can be a value of @ref
                                   QSPI_TimeOutActivation */
...
#define QSPI_SAMPLE_SHIFTING_HALFCYCLE                                                             \
    ((uint32_t)QUADSPI_CR_SSHIFT) /*!<1/2 clock cycle shift to sample data*/
/**
 * @}
 */
...
#define QSPI_CS_HIGH_TIME_1_CYCLE                                                                  \
    0x00000000U /*!<nCS stay high for at least 1 clock cycle between commands*/
#define QSPI_CS_HIGH_TIME_2_CYCLE                                                                  \
    ((uint32_t)QUADSPI_DCR_CSHT_0) /*!<nCS stay high for at least 2 clock cycles between           \
                                      commands*/
#define QSPI_CS_HIGH_TIME_3_CYCLE                                                                  \
    ((uint32_t)QUADSPI_DCR_CSHT_1) /*!<nCS stay high for at least 3 clock cycles between           \
                                      commands*/
#define QSPI_CS_HIGH_TIME_4_CYCLE                                                                  \
    ((uint32_t)QUADSPI_DCR_CSHT_0 |                                                                \
     QUADSPI_DCR_CSHT_1) /*!<nCS stay high for at least 4 clock cycles between commands*/
#define QSPI_CS_HIGH_TIME_5_CYCLE                                                                  \
    ((uint32_t)QUADSPI_DCR_CSHT_2) /*!<nCS stay high for at least 5 clock cycles between           \
                                      commands*/
#define QSPI_CS_HIGH_TIME_6_CYCLE                                                                  \
    ((uint32_t)QUADSPI_DCR_CSHT_2 |                                                                \
     QUADSPI_DCR_CSHT_0) /*!<nCS stay high for at least 6 clock cycles between commands*/
#define QSPI_CS_HIGH_TIME_7_CYCLE                                                                  \
    ((uint32_t)QUADSPI_DCR_CSHT_2 |                                                                \
     QUADSPI_DCR_CSHT_1) /*!<nCS stay high for at least 7 clock cycles between commands*/
...
#define QSPI_ALTERNATE_BYTES_1_LINE                                                                \
    ((uint32_t)QUADSPI_CCR_ABMODE_0) /*!<Alternate bytes on a single line*/
#define QSPI_ALTERNATE_BYTES_2_LINES                                                               \
    ((uint32_t)QUADSPI_CCR_ABMODE_1) /*!<Alternate bytes on two lines*/
#define QSPI_ALTERNATE_BYTES_4_LINES                                                               \
    ((uint32_t)QUADSPI_CCR_ABMODE) /*!<Alternate bytes on four lines*/
/**
 * @}
 */
...
#define QSPI_DDR_HHC_ANALOG_DELAY                                                                  \
    0x00000000U /*!<Delay the data output using analog delay in DDR mode*/
#if defined(QUADSPI_CCR_DHHC)
#define QSPI_DDR_HHC_HALF_CLK_DELAY                                                                \
    ((uint32_t)QUADSPI_CCR_DHHC) /*!<Delay the data output by one half of system clock in DDR      \
                                    mode*/
#endif
/**
 * @}
...
#define QSPI_SIOO_INST_ONLY_FIRST_CMD                                                              \
    ((uint32_t)QUADSPI_CCR_SIOO) /*!<Send instruction only for the first command*/
/**
 * @}
 */
...
#define QSPI_AUTOMATIC_STOP_DISABLE                                                                \
    0x00000000U /*!<AutoPolling stops only with abort or QSPI disabling*/
#define QSPI_AUTOMATIC_STOP_ENABLE                                                                 \
    ((uint32_t)QUADSPI_CR_APMS) /*!<AutoPolling stops as soon as there is a match*/
/**
 * @}
 */
...
#define QSPI_TIMEOUT_COUNTER_DISABLE                                                               \
    0x00000000U /*!<Timeout counter disabled, nCS remains                                          \
                   active*/
...
#define QSPI_FLAG_SM                                                                               \
    QUADSPI_SR_SMF /*!<Status match flag: received data matches in autopolling mode*/
#define QSPI_FLAG_FT                                                                               \
    QUADSPI_SR_FTF /*!<Fifo threshold flag: Fifo threshold reached or data left after read from    \
                      memory is complete*/
#define QSPI_FLAG_TC                                                                               \
...
#define IS_QSPI_SSHIFT(SSHIFT)                                                                     \
    (((SSHIFT) == QSPI_SAMPLE_SHIFTING_NONE) || ((SSHIFT) == QSPI_SAMPLE_SHIFTING_HALFCYCLE))
...
#define IS_QSPI_INSTRUCTION_MODE(MODE)                                                             \
    (((MODE) == QSPI_INSTRUCTION_NONE) || ((MODE) == QSPI_INSTRUCTION_1_LINE) ||                   \
     ((MODE) == QSPI_INSTRUCTION_2_LINES) || ((MODE) == QSPI_INSTRUCTION_4_LINES))
...
#define IS_QSPI_DDR_MODE(DDR_MODE)                                                                 \
    (((DDR_MODE) == QSPI_DDR_MODE_DISABLE) || ((DDR_MODE) == QSPI_DDR_MODE_ENABLE))
...
#define IS_QSPI_AUTOMATIC_STOP(APMS)                                                               \
    (((APMS) == QSPI_AUTOMATIC_STOP_DISABLE) || ((APMS) == QSPI_AUTOMATIC_STOP_ENABLE))
