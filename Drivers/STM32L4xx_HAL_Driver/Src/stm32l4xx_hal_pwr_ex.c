...
/**
  ...
  *          This parameter can be one of the following values:
  @if STM32L4S9xx
  *            @arg @ref PWR_REGULATOR_VOLTAGE_SCALE1_BOOST when available, Regulator voltage output
  range 1 boost mode,
 ...
 *          This parameter can be one of the following values:
 *            @arg @ref PWR_NO_SRAM2_RETENTION        SRAM2 is powered off in Standby mode (SRAM2
 * content is lost)
 *            @arg @ref PWR_FULL_SRAM2_RETENTION      Full SRAM2 is powered by the low-power
 * regulator in Standby mode
 *            @arg @ref PWR_4KBYTES_SRAM2_RETENTION   Only 4 Kbytes of SRAM2 is powered by the
 * low-power regulator in Standby mode
 ...
 * propagated only to the peripheral requesting it. SRAM1, SRAM2 and register contents are
 * preserved. SRAM3 content is preserved depending on RRSTP bit setting (not available on all
 * devices). The BOR is available. The voltage regulator is set in low-power mode but LPR bit must
