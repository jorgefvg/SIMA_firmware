...
#define STM32L4XX_HAL_VERSION                                                                      \
    ((STM32L4XX_HAL_VERSION_MAIN << 24U) | (STM32L4XX_HAL_VERSION_SUB1 << 16U) |                   \
...
__weak HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority) {
    ...
    /* Check uwTickFreq for MisraC 2012 (even if uwTickFreq is a enum type that doesn't take the
     * value zero)*/
 ...
 *          This parameter can be one of the following values:
 *            @arg SYSCFG_VREFBUF_HIGH_IMPEDANCE_DISABLE: VREF+ pin is internally connect to VREFINT
 * output.
