    ...
    *** Indirect functional mode ***
    ...
    [..]
      ...
      (#) If no data is required for the command, it is sent directly to the memory :
         ...
         (++) In interrupt mode, HAL_QSPI_CmdCpltCallback() will be called when the transfer is
 complete.
      (#) For the indirect write mode, use HAL_QSPI_Transmit(), HAL_QSPI_Transmit_DMA() or
         ...
         (++) In polling mode, the output of the function is done when the transfer is complete.
         (++) In interrupt mode, HAL_QSPI_FifoThresholdCallback() will be called when the fifo
 threshold is reached and HAL_QSPI_TxCpltCallback() will be called when the transfer is complete.
      ...
      (#) For the indirect read mode, use HAL_QSPI_Receive(), HAL_QSPI_Receive_DMA() or
         ...
         (++) In polling mode, the output of the function is done when the transfer is complete.
         (++) In interrupt mode, HAL_QSPI_FifoThresholdCallback() will be called when the fifo
 threshold is reached and HAL_QSPI_RxCpltCallback() will be called when the transfer is complete.
    ...
    *** Auto-polling functional mode ***
    ...
    [..]
      ...
      (#) After the configuration :
         (++) In polling mode, the output of the function is done when the status match is reached.
 The automatic stop is activated to avoid an infinite loop.
         (++) In interrupt mode, HAL_QSPI_StatusMatchCallback() will be called each time the status
 match is reached.
    ...
    *** Memory-mapped functional mode ***
    ...
    [..]
      ...
      (#) After the configuration, the QuadSPI will be used as soon as an access on the AHB is done
 on the address range. HAL_QSPI_TimeOutCallback() will be called when the timeout expires.
...
#define QSPI_FUNCTIONAL_MODE_INDIRECT_READ                                                         \
    ((uint32_t)QUADSPI_CCR_FMODE_0) /*!<Indirect read                                              \
                                       mode*/
#define QSPI_FUNCTIONAL_MODE_AUTO_POLLING                                                          \
    ((uint32_t)QUADSPI_CCR_FMODE_1) /*!<Automatic polling mode*/
...
static HAL_StatusTypeDef QSPI_WaitFlagStateUntilTimeout(QSPI_HandleTypeDef * hqspi, uint32_t Flag,
                                                        FlagStatus State, uint32_t Tickstart,
                                                        uint32_t Timeout);
    ...
    if (hqspi->State == HAL_QSPI_STATE_READY) {
        ...
        if (status == HAL_OK) {
            ...
            if (cmd->DataMode == QSPI_DATA_NONE) {
                /* When there is no data phase, the transfer start as soon as the configuration is
                done so wait until TC flag is set to go back in idle state */
                status =
                    QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_TC, SET, tickstart, Timeout);
...
HAL_StatusTypeDef HAL_QSPI_Command_IT(QSPI_HandleTypeDef * hqspi, QSPI_CommandTypeDef * cmd) {
    ...
    if (hqspi->State == HAL_QSPI_STATE_READY) {
        ...
        if (status == HAL_OK) {
            ...
            if (cmd->DataMode == QSPI_DATA_NONE) {
                /* When there is no data phase, the transfer start as soon as the configuration is
                done so activate TC and TE interrupts */
                /* Process unlocked */
                __HAL_UNLOCK(hqspi);
    ...
    if (hqspi->State == HAL_QSPI_STATE_READY) {
        ...
        if (pData != NULL) {
            ...
            while (hqspi->TxXferCount > 0U) {
                ...
                status =
                    QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_FT, SET, tickstart, Timeout);
...
HAL_StatusTypeDef HAL_QSPI_Abort(QSPI_HandleTypeDef * hqspi) {
    ...
    if (((uint32_t)hqspi->State & 0x2U) != 0U) {
        ...
        if (__HAL_QSPI_GET_FLAG(hqspi, QSPI_FLAG_BUSY) != RESET) {
            ...
            /* Wait until TC flag is set to go back in idle state */
            status =
                QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_TC, SET, tickstart, hqspi->Timeout);
