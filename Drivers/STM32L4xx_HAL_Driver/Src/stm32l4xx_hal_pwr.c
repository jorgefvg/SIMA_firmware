...
regulator on, reduced set of waking up IPs compared to Stop 1 mode.
      (+) Standby mode with SRAM2: all clocks are stopped except LSI and LSE, SRAM2 content
preserved, main regulator off, low power regulator on.
      (+) Standby mode without SRAM2: all clocks are stopped except LSI and LSE, main and low power
regulators off.
   ...
   ===============================
    [..]
      (+) Entry:
          ...
          (++) HAL_PWREx_EnterSTOP0Mode() for mode 0 or HAL_PWREx_EnterSTOP1Mode() for mode 1 or for
porting reasons HAL_PWR_EnterSTOPMode().
...
register. To enable this feature, the user can resort to HAL_PWREx_EnableSRAM2ContentRetention() API
                to set RRS bit.
   ...
   ======================
     [..]
      ...
      (+) Exit:
         ...
         [..] After waking up from Shutdown mode, program execution restarts in the same way as
after a Reset.
 ...
 *        SRAM1 and register contents are lost except for registers in the Backup domain and
 *        Standby circuitry. SRAM2 content can be preserved if the bit RRS is set in PWR_CR3
 * register. To enable this feature, the user can resort to HAL_PWREx_EnableSRAM2ContentRetention()
