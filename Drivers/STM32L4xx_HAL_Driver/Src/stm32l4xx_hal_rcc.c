...
/** @defgroup RCC_Exported_Functions_Group1 Initialization and de-initialization functions
 ...
 48MHZ. It can be used to generate the clock for the USB OTG FS (48 MHz). The number of flash wait
 ...
 source.
    ...
    [..] System, AHB and APB busses clocks configuration
         ...
         -@- All the peripheral clocks are derived from the System clock (SYSCLK) except:
           ...
           (+@) SAI: the SAI clock can be derived either from a specific PLL (PLLSAI1) or (PLLSAI2)
...
HAL_StatusTypeDef HAL_RCC_DeInit(void) {
    ...
    CLEAR_BIT(RCC->CR, RCC_CR_HSEON | RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_HSIASFS |
                           RCC_CR_PLLON | RCC_CR_PLLSAI1ON | RCC_CR_PLLSAI2ON);

#elif defined(RCC_PLLSAI1_SUPPORT)

    CLEAR_BIT(RCC->CR, RCC_CR_HSEON | RCC_CR_HSION | RCC_CR_HSIKERON | RCC_CR_HSIASFS |
                           RCC_CR_PLLON | RCC_CR_PLLSAI1ON);
...
HAL_StatusTypeDef HAL_RCC_OscConfig(RCC_OscInitTypeDef * RCC_OscInitStruct) {
    ...
    if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_MSI) == RCC_OSCILLATORTYPE_MSI) {
        ...
        /* Check if MSI is used as system clock or as PLL source when PLL is selected as system
         * clock */
            ...
            if ((READ_BIT(pll_config, RCC_PLLCFGR_PLLSRC) != RCC_OscInitStruct->PLL.PLLSource) ||
                ...
                if (sysclk_source != RCC_CFGR_SWS_PLL) {
...
#endif /* RCC_PLLSAI1_SUPPORT || RCC_PLLSAI2_SUPPORT */
                    {
                        ...
                        __HAL_RCC_PLL_CONFIG(
                            RCC_OscInitStruct->PLL.PLLSource, RCC_OscInitStruct->PLL.PLLM,
                            RCC_OscInitStruct->PLL.PLLN, RCC_OscInitStruct->PLL.PLLQ,
...
#if defined(RCC_PLLSAI2_SUPPORT)
                RCC->PLLCFGR &=
                    ~(RCC_PLLCFGR_PLLSRC | RCC_PLL_SYSCLK | RCC_PLL_48M1CLK | RCC_PLL_SAI3CLK);
#elif defined(RCC_PLLSAI1_SUPPORT)
                RCC->PLLCFGR &=
                    ~(RCC_PLLCFGR_PLLSRC | RCC_PLL_SYSCLK | RCC_PLL_48M1CLK | RCC_PLL_SAI2CLK);
#else
...
HAL_StatusTypeDef HAL_RCC_ClockConfig(RCC_ClkInitTypeDef * RCC_ClkInitStruct, uint32_t FLatency) {
    ...
    /* Apply higher HCLK prescaler request here to ensure CPU clock is not of of spec when SYSCLK is
     * increased */
    ...
    defined(STM32L4S7xx) || defined(STM32L4S9xx)
            ...
            if (RCC_GetSysClockFreqFromPLLSource() > 80000000U) {
                /* If lowest HCLK prescaler, apply intermediate step with HCLK prescaler 2 necessary
    ...
    defined(STM32L4R7xx) || defined(STM32L4R9xx) || defined(STM32L4S5xx) ||                        \
    defined(STM32L4S7xx) || defined(STM32L4S9xx)
            /* Overshoot management when going down from PLL as SYSCLK source and frequency above
             * 80Mhz */
            if (HAL_RCC_GetSysClockFreq() > 80000000U) {
                /* If lowest HCLK prescaler, apply intermediate step with HCLK prescaler 2 necessary
                 * before to go under 80Mhz */
                if (READ_BIT(RCC->CFGR, RCC_CFGR_HPRE) == RCC_SYSCLK_DIV1) {
                    MODIFY_REG(RCC->CFGR, RCC_CFGR_HPRE, RCC_SYSCLK_DIV2);
                    hpre = RCC_SYSCLK_DIV2;
                }
    ...
    /* Apply lower HCLK prescaler request here to ensure CPU clock is not of of spec when SYSCLK is
     * set */
    if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_HCLK) == RCC_CLOCKTYPE_HCLK) {
        if (RCC_ClkInitStruct->AHBCLKDivider < READ_BIT(RCC->CFGR, RCC_CFGR_HPRE)) {
            MODIFY_REG(RCC->CFGR, RCC_CFGR_HPRE, RCC_ClkInitStruct->AHBCLKDivider);
...
uint32_t HAL_RCC_GetSysClockFreq(void) {
    ...
    if (sysclk_source == RCC_CFGR_SWS_PLL) {
        ...
        */
        ...
        pllvco =
            (pllvco * (READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLN) >> RCC_PLLCFGR_PLLN_Pos)) / pllm;
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef * RCC_OscInitStruct) {
    ...
    RCC_OscInitStruct->PLL.PLLR =
        (((READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLR) >> RCC_PLLCFGR_PLLR_Pos) + 1U) << 1U);
#if defined(RCC_PLLP_SUPPORT)
#if defined(RCC_PLLP_DIV_2_31_SUPPORT)
