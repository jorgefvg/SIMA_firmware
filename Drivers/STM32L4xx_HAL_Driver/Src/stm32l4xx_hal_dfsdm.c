...
/** @defgroup DFSDM_Exported_Functions_Group1_Channel Channel initialization and de-initialization
functions
...
HAL_StatusTypeDef HAL_DFSDM_ChannelInit(DFSDM_Channel_HandleTypeDef * hdfsdm_channel) {
    ...
    if (v_dfsdm1ChannelCounter == 1U) {
        ...
        if (hdfsdm_channel->Init.OutputClock.Activation == ENABLE) {
            assert_param(
                IS_DFSDM_CHANNEL_OUTPUT_CLOCK_DIVIDER(hdfsdm_channel->Init.OutputClock.Divider));
    ...
    hdfsdm_channel->Instance->CHCFGR1 &=
        ~(DFSDM_CHCFGR1_DATPACK | DFSDM_CHCFGR1_DATMPX | DFSDM_CHCFGR1_CHINSEL);
    hdfsdm_channel->Instance->CHCFGR1 |=
        (hdfsdm_channel->Init.Input.Multiplexer | hdfsdm_channel->Init.Input.DataPacking |
    ...
    hdfsdm_channel->Instance->CHAWSCDR |=
        (hdfsdm_channel->Init.Awd.FilterOrder |
         ((hdfsdm_channel->Init.Awd.Oversampling - 1U) << DFSDM_CHAWSCDR_AWFOSR_Pos));

    /* Set channel offset and right bit shift */
    hdfsdm_channel->Instance->CHCFGR2 &= ~(DFSDM_CHCFGR2_OFFSET | DFSDM_CHCFGR2_DTRBS);
    hdfsdm_channel->Instance->CHCFGR2 |=
        (((uint32_t)hdfsdm_channel->Init.Offset << DFSDM_CHCFGR2_OFFSET_Pos) |
...
HAL_StatusTypeDef HAL_DFSDM_ChannelDeInit(DFSDM_Channel_HandleTypeDef * hdfsdm_channel) {
    ...
    a_dfsdm1ChannelHandle[DFSDM_GetChannelFromInstance(hdfsdm_channel->Instance)] =
        (DFSDM_Channel_HandleTypeDef *)NULL;
...
HAL_StatusTypeDef HAL_DFSDM_ChannelCkabStart(DFSDM_Channel_HandleTypeDef * hdfsdm_channel) {
    ...
    if (hdfsdm_channel->State != HAL_DFSDM_CHANNEL_STATE_READY) {
        ...
        while (
            (((DFSDM1_Filter0->FLTISR & DFSDM_FLTISR_CKABF) >> (DFSDM_FLTISR_CKABF_Pos + channel)) &
             1U) != 0U) {
...
HAL_StatusTypeDef HAL_DFSDM_ChannelScdStart(DFSDM_Channel_HandleTypeDef * hdfsdm_channel,
    ...
    if (hdfsdm_channel->State != HAL_DFSDM_CHANNEL_STATE_READY) {
        ...
        hdfsdm_channel->Instance->CHAWSCDR |=
            ((BreakSignal << DFSDM_CHAWSCDR_BKSCD_Pos) | Threshold);
...
HAL_StatusTypeDef HAL_DFSDM_ChannelPollForScd(const DFSDM_Channel_HandleTypeDef * hdfsdm_channel,
    ...
    if (hdfsdm_channel->State != HAL_DFSDM_CHANNEL_STATE_READY) {
        ...
        while (((DFSDM1_Filter0->FLTISR & DFSDM_FLTISR_SCDF) >>
                (DFSDM_FLTISR_SCDF_Pos + channel)) == 0U) {
            /* Check the Timeout */
            if (Timeout != HAL_MAX_DELAY) {
                if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U)) {
                    /* Return timeout status */
...
/** @defgroup DFSDM_Exported_Functions_Group1_Filter Filter initialization and de-initialization
functions
...
HAL_StatusTypeDef HAL_DFSDM_FilterInit(DFSDM_Filter_HandleTypeDef * hdfsdm_filter) {
    ...
    assert_param(
        IS_DFSDM_FILTER_INTEGRATOR_OVS_RATIO(hdfsdm_filter->Init.FilterParam.IntOversampling));
    ...
    hdfsdm_filter->Instance->FLTCR1 &=
        ~(DFSDM_FLTCR1_JSYNC | DFSDM_FLTCR1_JEXTEN | DFSDM_FLTCR1_JEXTSEL);
    if (hdfsdm_filter->Init.InjectedParam.Trigger == DFSDM_FILTER_EXT_TRIGGER) {
        assert_param(IS_DFSDM_FILTER_EXT_TRIG(hdfsdm_filter->Init.InjectedParam.ExtTrigger));
        assert_param(
            IS_DFSDM_FILTER_EXT_TRIG_EDGE(hdfsdm_filter->Init.InjectedParam.ExtTriggerEdge));
    ...
    hdfsdm_filter->Instance->FLTFCR |=
        (hdfsdm_filter->Init.FilterParam.SincOrder |
 ...
 * @param  CallbackID ID of the callback to be registered.
 *         This parameter can be one of the following values:
 *           @arg @ref HAL_DFSDM_FILTER_REGCONV_COMPLETE_CB_ID regular conversion complete callback
 * ID.
 *           @arg @ref HAL_DFSDM_FILTER_REGCONV_HALFCOMPLETE_CB_ID half regular conversion complete
 * callback ID.
 *           @arg @ref HAL_DFSDM_FILTER_INJCONV_COMPLETE_CB_ID injected conversion complete callback
 * ID.
 *           @arg @ref HAL_DFSDM_FILTER_INJCONV_HALFCOMPLETE_CB_ID half injected conversion complete
 * callback ID.
...
HAL_StatusTypeDef HAL_DFSDM_FilterConfigRegChannel(DFSDM_Filter_HandleTypeDef * hdfsdm_filter,
    ...
    if ((hdfsdm_filter->State != HAL_DFSDM_FILTER_STATE_RESET) &&
        ...
        if (ContinuousMode == DFSDM_CONTINUOUS_CONV_ON) {
            hdfsdm_filter->Instance->FLTCR1 |=
                (uint32_t)(((Channel & DFSDM_MSB_MASK) << DFSDM_FLTCR1_MSB_RCH_OFFSET) |
                           DFSDM_FLTCR1_RCONT);
        } else {
            hdfsdm_filter->Instance->FLTCR1 |=
                (uint32_t)((Channel & DFSDM_MSB_MASK) << DFSDM_FLTCR1_MSB_RCH_OFFSET);
...
HAL_StatusTypeDef HAL_DFSDM_FilterRegularStart_DMA(DFSDM_Filter_HandleTypeDef * hdfsdm_filter,
    ...
    /* Check DFSDM filter state */
        ...
        hdfsdm_filter->hdmaReg->XferHalfCpltCallback =
            (hdfsdm_filter->hdmaReg->Init.Mode == DMA_CIRCULAR) ? DFSDM_DMARegularHalfConvCplt
                                                                : NULL;
...
HAL_StatusTypeDef HAL_DFSDM_FilterRegularMsbStart_DMA(DFSDM_Filter_HandleTypeDef * hdfsdm_filter,
    ...
    /* Check DFSDM filter state */
        ...
        /* Start DMA in interrupt mode */
        if (HAL_DMA_Start_IT(hdfsdm_filter->hdmaReg,
                             (uint32_t)(&hdfsdm_filter->Instance->FLTRDATAR) + 2U, (uint32_t)pData,
                             Length) != HAL_OK) {
            /* Set DFSDM filter in error state */
            hdfsdm_filter->State = HAL_DFSDM_FILTER_STATE_ERROR;
            status = HAL_ERROR;
...
int32_t HAL_DFSDM_FilterGetRegularValue(const DFSDM_Filter_HandleTypeDef * hdfsdm_filter,
    ...
    *Channel = (reg & DFSDM_FLTRDATAR_RDATACH);
    ...
    /* So after applying a mask on these bits we have to perform a division by 256 (2 raised to the
     * power of 8) */
    ...
    if ((hdfsdm_filter->State != HAL_DFSDM_FILTER_STATE_INJ) &&
        ...
        if (hdfsdm_filter->InjConvRemaining == 0U) {
            ...
            hdfsdm_filter->InjConvRemaining = (hdfsdm_filter->InjectedScanMode == ENABLE)
                                                  ? hdfsdm_filter->InjectedChannelsNbr
                                                  : 1U;
        }
...
HAL_StatusTypeDef HAL_DFSDM_FilterInjectedStart_DMA(DFSDM_Filter_HandleTypeDef * hdfsdm_filter,
    ...
    /* Check DFSDM filter state */
        ...
        hdfsdm_filter->hdmaInj->XferHalfCpltCallback =
            (hdfsdm_filter->hdmaInj->Init.Mode == DMA_CIRCULAR) ? DFSDM_DMAInjectedHalfConvCplt
                                                                : NULL;
...
HAL_StatusTypeDef HAL_DFSDM_FilterInjectedMsbStart_DMA(DFSDM_Filter_HandleTypeDef * hdfsdm_filter,
    ...
    /* Check DFSDM filter state */
        ...
        /* Start DMA in interrupt mode */
        if (HAL_DMA_Start_IT(hdfsdm_filter->hdmaInj,
                             (uint32_t)(&hdfsdm_filter->Instance->FLTJDATAR) + 2U, (uint32_t)pData,
                             Length) != HAL_OK) {
            /* Set DFSDM filter in error state */
            hdfsdm_filter->State = HAL_DFSDM_FILTER_STATE_ERROR;
            status = HAL_ERROR;
...
HAL_StatusTypeDef HAL_DFSDM_FilterAwdStart_IT(DFSDM_Filter_HandleTypeDef * hdfsdm_filter,
    ...
    if ((hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_RESET) ||
        ...
        hdfsdm_filter->Instance->FLTAWHTR |=
            (((uint32_t)awdParam->HighThreshold << DFSDM_FLTAWHTR_AWHT_Pos) |
        ...
        hdfsdm_filter->Instance->FLTAWLTR |=
            (((uint32_t)awdParam->LowThreshold << DFSDM_FLTAWLTR_AWLT_Pos) |
...
void HAL_DFSDM_IRQHandler(DFSDM_Filter_HandleTypeDef * hdfsdm_filter) {
    ...
    /* Check if analog watchdog occurs */
        ...
        threshold = ((reg & DFSDM_FLTAWSR_AWLTF) != 0U) ? DFSDM_AWD_LOW_THRESHOLD
        ...
        while (channel < DFSDM1_CHANNEL_NUMBER) {
            ...
            if (((reg & 1U) != 0U) && (a_dfsdm1ChannelHandle[channel] != NULL)) {
                ...
                if ((a_dfsdm1ChannelHandle[channel]->Instance->CHCFGR1 & DFSDM_CHCFGR1_CKABEN) !=
                    0U) {
                    /* Clear clock absence flag */
                    hdfsdm_filter->Instance->FLTICR =
                        (1UL << (DFSDM_FLTICR_CLRCKABF_Pos + channel));
...
static void DFSDM_DMARegularHalfConvCplt(DMA_HandleTypeDef * hdma) {
    ...
    DFSDM_Filter_HandleTypeDef * hdfsdm_filter =
        (DFSDM_Filter_HandleTypeDef *)((DMA_HandleTypeDef *)hdma)->Parent;
...
#endif /* STM32L451xx || STM32L452xx || STM32L462xx || STM32L471xx || STM32L475xx || STM32L476xx   \
          || STM32L485xx || STM32L486xx || STM32L496xx || STM32L4A6xx || STM32L4R5xx ||            \
          STM32L4R7xx || STM32L4R9xx || STM32L4S5xx || STM32L4S7xx || STM32L4S9xx || STM32L4P5xx   \
          || STM32L4Q5xx */
