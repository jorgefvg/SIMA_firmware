...
#define LITE_INSPECT_CB(flags, node_id, data_ptr, data_size, data_fmt, data_id, data_pos)          \
    {                                                                                              \
        if (graph->cb) {                                                                           \
            graph->cb((const void *)(graph->cb_cookie), (const stai_flags)(flags),                 \
                      (const int32_t)(node_id), (const void *)(data_ptr),                          \
                      (const int32_t)(data_size), (const int32_t)(data_fmt),                       \
                      (const int32_t)(data_id), (const int32_t)(data_pos));                        \
        }                                                                                          \
    }
