...
#define AI_STORAGE_KLASS_PACK(type_, dim_, ...) AI_PACK_STORAGE_ARRAY(type_, dim_, __VA_ARGS__)

#define AI_STORAGE_KLASS_INIT(type_, size_, data_)                                                 \
    { .type = (type_), .size = (size_), .data = (ai_handle)(data_), }
...
#define AI_SHAPE_INIT(dim_, ...)                                                                   \
    AI_STORAGE_KLASS_INIT(AI_STORAGE_KLASS_SHAPE, dim_,                                            \
                          AI_STORAGE_KLASS_PACK(ai_shape_dimension, dim_, ##__VA_ARGS__))

#define AI_SHAPE_INIT_FROM_BUFFER(dim_, buffer_)                                                   \
    AI_STORAGE_KLASS_INIT(AI_STORAGE_KLASS_SHAPE, dim_, buffer_)

#define AI_SHAPE_ALLOCATE_STATIC(num_dim_) AI_SHAPE_INIT((num_dim_), 0)
...
#define AI_STRIDE_INIT(dim_, ...)                                                                  \
    AI_STORAGE_KLASS_INIT(AI_STORAGE_KLASS_STRIDE, dim_,                                           \
                          AI_STORAGE_KLASS_PACK(ai_stride_dimension, dim_, ##__VA_ARGS__))

#define AI_STRIDE_INIT_FROM_BUFFER(dim_, buffer_)                                                  \
    AI_STORAGE_KLASS_INIT(AI_STORAGE_KLASS_STRIDE, dim_, buffer_)

#define AI_STRIDE_ALLOCATE_STATIC(num_dims_) AI_STRIDE_INIT((num_dims_), 0)
...
#define AI_SIZE(value_) AI_CAST(ai_size, value_)

/*!  AI_KLASS_OBJ HANDLERS SECTION         ************************************/
#define AI_KLASS_OBJ(obj_) AI_CAST(ai_klass_obj, obj_)

/*!  GENERIC HANDLERS SECTION              ************************************/
#define AI_OBJ_DATA(obj_, type_) AI_CAST(type_, (obj_)->data)

/*!  AI_BUFFER HANDLERS SECTION            ************************************/
#define AI_BUFFER_OBJ(ptr_) AI_CAST(ai_buffer *, ptr_)

/*!  AI_ARRAY HANDLERS SECTION             ************************************/
#define AI_ARRAY_OBJ(ptr_) AI_CAST(ai_array *, ptr_)

#define AI_ARRAY_OBJ_INIT_STATIC(type_, format_, size_, ...)                                       \
    {                                                                                              \
        .format = AI_FMT_OBJ(format_), .size = (ai_array_size)(size_),                             \
        .data = (ai_ptr)((type_[]){__VA_ARGS__}), .data_start = AI_PTR(0),                         \
    }

#define AI_ARRAY_OBJ_INIT(format_, data_, data_start_, size_)                                      \
    {                                                                                              \
        .format = AI_FMT_OBJ(format_), .size = AI_CAST(ai_array_size, size_),                      \
        .data = AI_PTR(data_), .data_start = AI_PTR(data_start_)                                   \
    }
...
#define AI_ARRAY_LIST_OBJ_INIT(arrays_ptr_) ((ai_array *)(arrays_ptr_))

#define AI_ARRAY_LIST_FLAGS(list_)          ((list_) ? (list_)->flags : 0x0)

#define AI_ARRAY_LIST_SIZE(list_)           ((list_) ? (list_)->size : 0)

#define AI_ARRAY_LIST_DATA(list_, pos_)     ((list_) ? &((list_)->data[pos_]) : NULL)

/********************************* ai_tensor macros  **************************/
#define AI_TENSOR_OBJ(obj_) AI_CAST(ai_tensor *, obj_)

#define AI_TENSOR_INFO_OBJ_INIT(id_, flags_, data_size_)                                           \
    { .id = (id_), .flags = (flags_), .data_size = (data_size_) }

#define AI_TENSOR_OBJ_INIT(id_, flags_, shape_, stride_, arrays_size_, arrays_ptr_, klass_obj_)    \
    {                                                                                              \
        .klass = (ai_klass_obj)(klass_obj_),                                                       \
        .info = AI_TENSOR_INFO_OBJ_INIT(id_, flags_, arrays_size_), .shape = shape_,               \
        .stride = stride_, .data = AI_ARRAY_LIST_OBJ_INIT(AI_PACK(arrays_ptr_)),                   \
    }

#define AI_TENSOR_OBJ_DECLARE(name_, attr_, id_, flags_, shape_, stride_, arrays_size_,            \
                              arrays_ptr_, klass_obj_)                                             \
    AI_ALIGNED(4)                                                                                  \
    attr_ ai_tensor name_ =                                                                        \
        AI_TENSOR_OBJ_INIT(id_, flags_, AI_PACK(shape_), AI_PACK(stride_), arrays_size_,           \
                           AI_PACK(arrays_ptr_), AI_PACK(klass_obj_));
...
#pragma message(                                                                                   \
    "Including deprecated AI_TENSOR_LIST_ENTRY, AI_TENSOR_LIST_EMPTY, AI_TENSOR_LIST_IO_ENTRY")

AI_DEPRECATED
#define AI_TENSOR_LIST_EMPTY AI_TENSOR_LIST_OBJ_EMPTY
...
#define AI_TENSOR_LIST_OBJ_INIT(flags_, size_, ...)                                                \
    { .size = (size_), .flags = (flags_), .tensor = (ai_tensor *[]){__VA_ARGS__}, .info = NULL }

#define AI_TENSOR_LIST_OBJ_EMPTY                                                                   \
    { .size = 0, .flags = AI_FLAG_NONE, .tensor = (ai_tensor *[]){NULL}, .info = NULL }

#define AI_TENSOR_LIST_OBJ_DECLARE(name_, attr_, flags_, size_, ...)                               \
    AI_ALIGNED(4)                                                                                  \
    attr_ ai_tensor_list name_ = AI_TENSOR_LIST_OBJ_INIT(flags_, size_, __VA_ARGS__);

/********************************* TENSOR LIST I/O MACROS  ********************/

#define AI_TENSOR_LIST_IO_OBJ_INIT(flags_, size_, ...)                                             \
    {                                                                                              \
        .size = (size_), .flags = (flags_), .tensor = (ai_tensor *[]){__VA_ARGS__},                \
        .info = (ai_tensor_list_info[1]) {                                                         \
            {                                                                                      \
                .buffer = (ai_buffer[size_]){AI_STRUCT_INIT},                                      \
                .state = (ai_tensor_state[size_]){AI_STRUCT_INIT},                                 \
                .meta = (ai_buffer_meta_info[size_]) {                                             \
                    AI_STRUCT_INIT                                                                 \
                }                                                                                  \
            }                                                                                      \
        }                                                                                          \
...
#define AI_TENSOR_CHAIN_OBJ_INIT(flags_, size_, ...)                                               \
    {                                                                                              \
        .size = (size_), .flags = (flags_), .chain = (ai_tensor_list[]) { __VA_ARGS__ }            \
    }

#define AI_TENSOR_CHAIN_OBJ_DECLARE(name_, attr_, size_, ...)                                      \
    AI_ALIGNED(4)                                                                                  \
    attr_ ai_tensor_chain name_ = AI_TENSOR_CHAIN_OBJ_INIT(AI_FLAG_NONE, size_, __VA_ARGS__);

/********************************* TENSOR CHAIN I/O MACROS  *******************/
#define AI_TENSOR_CHAIN_IO_OBJ_INIT(flags_, in_tensor_list_, out_tensor_list_)                     \
    { .chain = (ai_tensor_list[]){in_tensor_list_, out_tensor_list_}, .size = 2, .flags = (flags_) }

#define AI_TENSOR_CHAIN_IO_OBJ_DECLARE(name_, attr_, flags_, in_tensor_list_, out_tensor_list_)    \
    AI_ALIGNED(4)                                                                                  \
    attr_ ai_tensor_chain_io name_ =                                                               \
        AI_TENSOR_CHAIN_IO_OBJ_INIT(flags_, in_tensor_list_, out_tensor_list_);

/*******************************  NETWORK SECTION  ****************************/
#define AI_NETWORK_OBJ(obj_) ((ai_network *)(obj_))

#if (AI_TOOLS_API_VERSION < AI_TOOLS_API_VERSION_1_5)

AI_DEPRECATED
#define AI_NETWORK_OBJ_INIT(weights_buffer_, activations_buffer_, in_tensor_list_ptr_,             \
                            out_tensor_list_ptr_, in_node_ptr_, signature_, klass_obj_)            \
    {                                                                                              \
        .magic = AI_MAGIC_CONTEXT_TOKEN, .signature = signature_, .tool_api_version = 0x0,         \
        .error = AI_ERROR_INIT(NONE, NONE), .flags = AI_FLAG_NONE,                                 \
        .klass = AI_KLASS_OBJ(klass_obj_), .n_batches = 0, .batch_id = 0,                          \
        ...
        .tensors = AI_TENSOR_CHAIN_IO_OBJ_INIT(AI_FLAG_NONE, AI_PACK(in_tensor_list_ptr_),         \
                                               AI_PACK(out_tensor_list_ptr_)),                     \
        .input_node = AI_NODE_OBJ(in_node_ptr_), .current_node = AI_NODE_OBJ(NULL),                \
        .on_node_exec = NULL, .data_exec = NULL, .lite_cb = NULL,                                  \
    }
...
#define AI_NETWORK_OBJ_INIT(weights_buffer_, activations_buffer_, in_tensor_list_ptr_,             \
    ...
    {                                                                                              \
        .magic = AI_MAGIC_CONTEXT_TOKEN, .signature = signature_, .tool_api_version = 0x0,         \
        .error = AI_ERROR_INIT(NONE, NONE), .klass = AI_KLASS_OBJ(klass_obj_),                     \
        .flags = AI_FLAG_NONE, .n_batches = 0, .batch_id = 0,                                      \
        .buffers =                                                                                 \
            AI_NETWORK_BUFFERS_INIT(AI_PACK(weights_buffer_), AI_PACK(activations_buffer_)),       \
        .tensors = AI_TENSOR_CHAIN_IO_OBJ_INIT(AI_FLAG_NONE, AI_PACK(in_tensor_list_ptr_),         \
                                               AI_PACK(out_tensor_list_ptr_)),                     \
        .input_node = AI_NODE_OBJ(in_node_ptr_), .current_node = AI_NODE_OBJ(NULL),                \
        .on_node_exec = NULL, .data_exec = NULL, .lite_cb = NULL,                                  \
...
#define AI_NETWORK_OBJ_DECLARE(name_, attr_, weights_buffer_, activations_buffer_,                 \
                               in_tensor_list_ptr_, out_tensor_list_ptr_, in_node_ptr_,            \
                               signature_, klass_obj_)                                             \
    AI_ALIGNED(4)                                                                                  \
    attr_ ai_network name_ = AI_NETWORK_OBJ_INIT(                                                  \
        AI_PACK(weights_buffer_), AI_PACK(activations_buffer_), AI_PACK(in_tensor_list_ptr_),      \
        AI_PACK(out_tensor_list_ptr_), (in_node_ptr_), (signature_), (klass_obj_));

#define AI_NETWORK_ACQUIRE_CTX(handle_) AI_NETWORK_OBJ(ai_platform_context_acquire(handle_))
 ...
 */
AI_PACKED_STRUCT_START
typedef AI_ALIGNED_TYPE(struct, 4) AI_PACKED{AI_CONTEXT_FIELDS} ai_context;
AI_PACKED_STRUCT_END
...
#define AI_INTQ_INFO_OBJ_INIT(flags_, scale_, zeropoint_)                                          \
    { .scale = (scale_), .zeropoint = (ai_handle)(zeropoint_), .flags = (flags_), }
...
#define AI_PACK_INTQ_INFO(scale_, zp_)                                                             \
    (INTQ_CONST ai_intq_info[1]) {                                                                 \
        { .scale = (INTQ_CONST ai_float *)AI_PACK(scale_), .zeropoint = (ai_handle)AI_PACK(zp_) }  \
    }
...
#define AI_INTQ_INFO_LIST_OBJ_INIT(flags_, size_, info_)                                           \
    { .flags = (flags_), .size = (size_), .info = (info_), }

#define AI_INTQ_INFO_LIST_OBJ_EMPTY                                                                \
    { 0 }

#define AI_INTQ_INFO_LIST_OBJ_DECLARE(name_, attr_, flags_, size_, info_)                          \
    AI_ALIGNED(4)                                                                                  \
    attr_ ai_intq_info_list name_ = AI_INTQ_INFO_LIST_OBJ_INIT(flags_, size_, AI_PACK(info_));
...
typedef AI_ALIGNED_TYPE(struct, 4) AI_PACKED ai_observer_node_s {
    ...
    const ai_tensor_chain *
        tensors; /*!< pointer to a 4 sized array see @ref ai_tensor_chain_type */
}
ai_observer_node;
AI_PACKED_STRUCT_END
...
typedef ai_u32 (*ai_observer_node_cb)(const ai_handle cookie, const ai_u32 flags,
...
typedef ai_u32 (*ai_node_exec_cb)(const ai_node_exec_state state, struct ai_node_s * cur,
                                  const ai_handle ctx);
...
typedef AI_ALIGNED_TYPE(struct, 4) AI_PACKED {
    ...
    // New 6.1 context storing explicitly network buffers. This allow also management of network
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_network_set_error(ai_context * net_ctx, const ai_error_type type,
 ...
 */
AI_INTERFACE_TYPE
ai_error ai_platform_network_create(ai_handle * network, const ai_buffer * network_config,
                                    ai_context * net_ctx, const ai_u8 tool_major,
                                    const ai_u8 tool_minor, const ai_u8 tool_micro);
 ...
 */
AI_INTERFACE_TYPE
ai_context * ai_platform_network_init(ai_handle network, const ai_network_params * params);
 ...
 */
AI_INTERFACE_TYPE
ai_i32 ai_platform_network_process(ai_handle network, const ai_buffer * input, ai_buffer * output);

/****************************************************************************
 ** ST.AI Wrapper APIs
 ****************************************************************************/
AI_INTERFACE_TYPE
void ai_platform_stai_handle_error(ai_error * dst_error, const stai_return_code code);

AI_INTERFACE_TYPE
ai_buffer * ai_platform_stai_bind_io(ai_u16 * n_buffer, ai_buffer * dst_io_buffers,
                                     const stai_ptr * src_io_buffers,
                                     const stai_size src_io_buffers_size);

AI_INTERFACE_TYPE
ai_bool ai_platform_stai_update_io(ai_i32 * batch_id, stai_ptr * dst_inputs, stai_ptr * dst_outputs,
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_observer_node_info(ai_handle network, ai_observer_node * node_info);
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_observer_register(ai_handle network, ai_observer_node_cb cb, ai_handle cookie,
                                      ai_u32 flags);

AI_INTERFACE_TYPE
ai_bool ai_platform_observer_register_s(ai_handle network, ai_observer_exec_ctx * ctx);
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_observer_unregister(ai_handle network, ai_observer_node_cb cb,
                                        ai_handle cookie);

AI_INTERFACE_TYPE
ai_bool ai_platform_observer_unregister_s(ai_handle network, ai_observer_exec_ctx * ctx);
