...
/******************************************************************************/
#ifdef HAS_AI_ASSERT
#define ASSERT_ARRAY_SANITY(a_) AI_ASSERT((a_) && (a_)->size > 0)
...
#define ASSERT_TENSOR_LIST_SANITY(tlist_) AI_ASSERT((tlist_) && (GET_TENSOR_LIST_SIZE(tlist_) > 0))
...
/******************************************************************************/
#define AI_NODE_TYPE(type_) ((ai_node_type)((ai_u32)(type_)&0xFFFF))

#define AI_NODE_OBJ(obj_)   ((ai_node *)(obj_))

#define AI_NODE_FUNC(func_) ((node_func)(func_))
...
#define AI_NODE_COMMON_INIT(type_, id_, flags_, klass_, network_, next_, forward_)                 \
    .type = AI_NODE_TYPE(type_), .id = AI_ID_OBJ(id_), .flags = (flags_),                          \
    .klass = AI_KLASS_OBJ(klass_), .network = AI_NETWORK_OBJ(network_),                            \
    .next = AI_NODE_OBJ(next_), .forward = AI_NODE_FUNC(forward_)
...
#define AI_FOR_EACH_TENSOR_LIST_DO(idx_, t_ptr_, tlist_ptr_)                                       \
    ...
    for (ai_size idx_ = 0; (idx_ < GET_TENSOR_LIST_SIZE(tlist_ptr_)) &&                            \
                           ((t_ptr_ = GET_TENSOR_LIST_ITEM(tlist_ptr_, idx_)) != NULL);            \
         ++idx_)

#define GET_TENSOR_LIST_INFO(list_)         ((list_)->info)

#define GET_TENSOR_LIST_META(list_, pos_)   (&(GET_TENSOR_LIST_INFO(list_)->meta[pos_]))

#define GET_TENSOR_LIST_STATE(list_, pos_)  (&(GET_TENSOR_LIST_INFO(list_)->state[pos_]))

#define GET_TENSOR_LIST_BUFFER(list_, pos_) (&(GET_TENSOR_LIST_INFO(list_)->buffer[pos_]))

#define GET_TENSOR_LIST_ITEM(list_, pos_)                                                          \
    ((NULL != GET_TENSOR_LIST_ITEMS(list_)) ? GET_TENSOR_LIST_ITEMS(list_)[(pos_)] : NULL)

#define GET_TENSOR_LIST_ITEMS(list_)  ((list_)->tensor)

#define GET_TENSOR_LIST_SIZE(list_)   ((NULL != (list_)) ? (list_)->size : 0)

#define GET_TENSOR_CHAIN_SIZE(chain_) ((NULL != (chain_)) ? (chain_)->size : 0)
...
#define GET_TENSOR_LIST_IN(chain_)      (GET_TENSOR_LIST(chain_, INPUT))

#define GET_TENSOR_LIST_OUT(chain_)     (GET_TENSOR_LIST(chain_, OUTPUT))

#define GET_TENSOR_LIST_WEIGTHS(chain_) (GET_TENSOR_LIST(chain_, WEIGHTS))

#define GET_TENSOR_LIST_SCRATCH(chain_) (GET_TENSOR_LIST(chain_, SCRATCH))

#define GET_TENSOR_IN(chain_, pos_)     (GET_TENSOR_LIST_ITEM(GET_TENSOR_LIST_IN(chain_), (pos_)))

#define GET_TENSOR_OUT(chain_, pos_)    (GET_TENSOR_LIST_ITEM(GET_TENSOR_LIST_OUT(chain_), (pos_)))
 ...
 */
typedef AI_ALIGNED_TYPE(struct, 4) ai_node_s{AI_NODE_COMMON_FIELDS_DECLARE} ai_node;
 ...
 */
AI_INTERNAL_API
ai_bool core_set_error(ai_error * error, const ai_error_type type, const ai_error_code code);
