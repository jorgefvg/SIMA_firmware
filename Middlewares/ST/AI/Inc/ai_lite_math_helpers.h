...
#define AI_FLOAT_TOLERANCE                                                                         \
    (6.19209290e-5F) /* Used for small calculation                                                 \
                        noise issues */
...
#define AI_MATH_TANH(x)                           tanhf(x)
#define AI_MATH_SQUARE(x)                         AI_MATH_POW(x, 2.0f)
#define AI_MATH_RELU_TEST(x, thr, min, max)       (((x) <= (thr)) ? (min) : (max))

#define AI_MATH_CLIP_LINEAR_REMAP(x, alpha, beta) (AI_MAX(0, AI_MIN(1, ((x) * (alpha) + (beta)))))
...
#define AI_MATH_RELU_THRESHOLDED(x, thr) AI_MATH_RELU_TEST(x, thr, 0, (x))

#define AI_MATH_LEAKY_RELU(x, neg_slope, pos_slope)                                                \
    AI_MATH_RELU_TEST(x, 0, (x) * (neg_slope), (x) * (pos_slope))
//          ( ((x)>0) ? (x)*(pos_slope) : (x)*(neg_slope) )

#define AI_MATH_PRELU(x, slope) AI_MATH_RELU_TEST(x, 0, (x) * (slope), (x))
// AI_MATH_LEAKY_RELU(x, slope, 1)

#define AI_MATH_RELU(x) AI_MATH_RELU_TEST(x, 0, 0, x)
//  AI_MAX(x, 0)

#define AI_MATH_ELU(x, alpha)               (AI_MAX(0.0f, (x)) + AI_MIN(0.0f, (alpha) * (AI_MATH_EXP(x) - 1.0f)))

#define AI_MATH_SELU(x, alpha, scale)       ((scale)*AI_MATH_ELU(x, alpha))

#define AI_MATH_SCALED_TANH(x, alpha, beta) ((alpha)*AI_MATH_TANH((beta) * (x)))
...
#define AI_MATH_HARD_SIGMOID(x, alpha, beta) AI_MATH_CLIP_LINEAR_REMAP(x, alpha, beta)

#define AI_MATH_GELU_NO_APPROXIMATE(x)       ((x / 2.0f) * (1.0f + AI_MATH_ERF(x / AI_MATH_SQRT(2.0f))))

#define AI_MATH_GELU_APPROXIMATE(x)                                                                \
    ((x / 2.0f) *                                                                                  \
     (1.0f + AI_MATH_TANH(AI_MATH_SQRT(2.0f / PI) * (x + 0.044715f * AI_MATH_POW(x, 3.0f)))))
...
#define AI_MATH_SWISH(x)      ((x)*AI_MATH_SIGMOID(x))

#define AI_MATH_HARD_SWISH(x) ((x)*AI_MATH_CLIP_LINEAR_REMAP(x, 1.0f / 6, 0.5f))

#define AI_MATH_SOFT_PLUS(x)  AI_MATH_LOG(1.0f + AI_MATH_EXP(x))

#define AI_MATH_SOFT_SIGN(x)  ((x) / (1.0f + AI_ABS(x)))
...
#define AI_VEC4_FLOAT(ptr_) _get_vec4_float((ai_handle)(ptr_))
 ...
 */
AI_INTERFACE_ENTRY
void st_mat_init_f32(ai_matrix_f32 * S, const uint16_t nRows, const uint16_t nColumns,
                     float * pData);
 ...
 */
AI_INTERFACE_ENTRY
uint32_t st_mat_gemm_f32(const ai_matrix_f32 * pSrcA, const ai_matrix_f32 * pSrcB,
                         const ai_matrix_f32 * pSrcC, const float alpha, const float beta,
                         const int8_t tA, const int8_t tB, ai_matrix_f32 * pDstY);
