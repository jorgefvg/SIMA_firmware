...
/*****************************************************************************/
#define STAI_PACK(...) __VA_ARGS__

#define STAI_UNUSED(x) (void)(x);
...
/*  Get format type */
#define STAI_FORMAT_GET_TYPE(format) ((stai_format_type)(((format) >> 17) & 0xF))

/*  Get format sign : 0 or 1 allowed */
#define STAI_FORMAT_GET_SIGN(format) (((format) >> 23) & 0x1)

/*  Get format bits - including sign, fractional bits. padding bits are not included */
#define STAI_FORMAT_GET_BITS(format) (((format) >> 7) & 0x7F)
...
#define STAI_FORMAT_GET_FBITS(format) ((((format) >> 0) & 0x7F) - 64)
...
/* buffer address field `buffer_descriptor::address` may not be used by the user and functions
   `get_buffer_address()`, `set_buffer_address()` must be used instead */
...
/* return codes >= STAI_ERROR_GENERIC are errors.
   all codes < STAI_ERROR_GENERIC are available for managing extended set of implementation specific
   return codes APIs track only 1 error: the 1st one generated while calling the APIs. No multiple
   error handling considered. NOTE: need also to provide proper handling of a new return code in
   stai_get_return_code_name() implementation.
*/
typedef enum {
 ...
 **/
typedef void (*stai_event_cb)(void * cb_cookie, const stai_event_type event_type,
...
STAI_API_ENTRY
stai_format stai_init_format(const stai_format_type type, const int8_t sign, const int32_t bits,
                             const int32_t fbits);
