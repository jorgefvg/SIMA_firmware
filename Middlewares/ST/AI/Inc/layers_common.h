...
#define AI_LAYER_OBJ_INIT(type_, id_, network_, next_, forward_, ...)                              \
    {                                                                                              \
        AI_NODE_COMMON_INIT(AI_CONCAT(AI_LAYER_, type_), id_, 0x0, NULL, network_, next_,          \
                            forward_),                                                             \
            ##__VA_ARGS__                                                                          \
    }

AI_DEPRECATED
#define AI_LAYER_OBJ_DECLARE(varname_, id_, type_, struct_, forward_func_, network_, next_, attr_, \
                             ...)                                                                  \
    AI_ALIGNED(4)                                                                                  \
...
#define AI_LAYER_OBJ_INIT(type_, id_, flags_, klass_, network_, next_, forward_, tensors_, ...)    \
    {                                                                                              \
        AI_NODE_COMMON_INIT(AI_CONCAT(AI_LAYER_, type_), id_, flags_, klass_, network_, next_,     \
                            forward_),                                                             \
            .tensors = (tensors_), ##__VA_ARGS__                                                   \
    }

#define AI_LAYER_OBJ_DECLARE(varname_, id_, type_, flags_, klass_obj_, struct_, forward_func_,     \
                             tensors_chain_, network_, next_, attr_, ...)                          \
    AI_ALIGNED(4)                                                                                  \
    attr_ AI_CONCAT(ai_layer_, struct_) varname_ =                                                 \
        AI_LAYER_OBJ_INIT(type_, id_, flags_, klass_obj_, network_, next_, forward_func_,          \
                          tensors_chain_, ##__VA_ARGS__);
 ...
 */
AI_INTERNAL_API
void ai_layer_check_scratch_size(ai_layer_type layer_type, ai_array_format fmt, ai_size filt_width,
                                 ai_size filt_height, ai_u16 n_channel_in, ai_u16 n_channel_out,
                                 ai_bool is_pointwise, ai_bool is_rgb, ai_bool is_depthwise,
                                 ai_bool is_ch1st, ai_bool is_ch_wise, ai_bool is_sssa,
                                 ai_u32 tensor_scratch_size_bytes, const char * p_function_name,
                                 const int line_nb);

#ifdef HAS_AI_ASSERT
#define CHECK_SCRATCH_BUFFER_SIZE(layer_type, fmt, filt_width, filt_height, n_channel_in,          \
                                  n_channel_out, is_pointwise, is_rgb, is_depthwise, is_ch1st,     \
                                  is_ch_wise, is_sssa_ch, tensor_scratch_size_bytes)               \
    ai_layer_check_scratch_size(layer_type, fmt, filt_width, filt_height, n_channel_in,            \
                                n_channel_out, is_pointwise, is_rgb, is_depthwise, is_ch1st,       \
                                is_ch_wise, is_sssa_ch, tensor_scratch_size_bytes, __FUNCTION__,   \
                                __LINE__);
#else
#define CHECK_SCRATCH_BUFFER_SIZE(layer_type, fmt, filt_width, filt_height, n_channel_in,          \
                                  n_channel_out, is_pointwise, is_rgb, is_depthwise, is_ch1st,     \
