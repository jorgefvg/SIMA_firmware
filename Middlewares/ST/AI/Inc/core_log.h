...
#define AI_LOG_ACQUIRE()                ai_log_acquire()
#define AI_LOG_SET_LEVEL(level_)        AI_WRAP_FUNC(ai_log_set_level(level_);)
#define AI_LOG_SET_QUIET(onoff_)        AI_WRAP_FUNC(ai_log_set_quiet(onoff_);)
#define AI_LOG_SET_LOCK_FN(fn_, udata_) AI_WRAP_FUNC(ai_log_set_lock(fn_, udata_);)
#define AI_LOG_CHANNEL_PUSH(level_, fn_, udata_)                                                   \
    AI_WRAP_FUNC(ai_log_channel_push(level_, fn_, udata_);)
#define AI_LOG_CHANNEL_POP(fn_, udata_) AI_WRAP_FUNC(ai_log_channel_pop(fn_, udata_);)
#ifdef LOG_USE_FILE
#define AI_LOG_SET_FILE_POINTER(fp_) AI_WRAP_FUNC(ai_log_set_fp(fp_);)
#else
...
#if defined HAS_LOG
#define AI_LOG_PRINT(level, fmt, ...) AI_WRAP_FUNC(ai_log_print(level, fmt, ##__VA_ARGS__);)
#else
#define AI_LOG_PRINT(level, fmt, ...) AI_WRAP_FUNC(/*AI_LOG_PRINT(...)*/)
#endif
