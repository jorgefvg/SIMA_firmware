...
#define AI_FMT_BINARY_MASK        (AI_FMT_MASK & (~(_FMT_SIGN_MASK << _FMT_SIGN_BITS)))

#define AI_FMT_IS_BINARY(val_)    (((val_)&AI_FMT_BINARY_MASK) == AI_ARRAY_FORMAT_U1)

#define AI_FMT_GET(val_)          ((AI_FMT_OBJ(val_)) & AI_FMT_MASK)

#define AI_FMT_MASK_Q(val_)       (AI_FMT_OBJ(val_) & (~(AI_FMT_QMASK)))

#define AI_FMT_GET_Q(val_)        (AI_FMT_MASK_Q(val_) | AI_FMT_SET_BITS(0) | AI_FMT_SET_FBITS(0))

#define AI_FMT_GET_FLAGS(val_)    (((AI_FMT_OBJ(val_)) & (~AI_FMT_MASK)) >> AI_FMT_FLAG_BITS)

#define AI_FMT_SAME(fmt1_, fmt2_) (AI_FMT_GET(fmt1_) == AI_FMT_GET(fmt2_))
...
#define AI_FMT_SET_BITS(val)      _FMT_SET((val) + _FMT_BITS_BIAS, _FMT_BITS_MASK, _FMT_BITS_BITS)
#define AI_FMT_GET_BITS(fmt)      ((ai_i8)_FMT_GET(fmt, _FMT_BITS_MASK, _FMT_BITS_BITS) - _FMT_BITS_BIAS)
#define AI_FMT_SET_FBITS(val)     _FMT_SET((val) + _FMT_FBITS_BIAS, _FMT_FBITS_MASK, _FMT_FBITS_BITS)
 ...
 */
#define AI_FMT_GET_BITS_SIZE(fmt_) AI_FMT_GET_BITS(fmt_)
...
#define AI_BUFFER_FMT_MASK_Q(fmt_) (AI_BUFFER_FMT_OBJ(fmt_) & 0xFFFFC000)

#define AI_BUFFER_FMT_GET_Q(fmt_)                                                                  \
    (AI_BUFFER_FMT_MASK_Q(fmt_) | AI_BUFFER_FMT_SET_FBITS(0) | AI_BUFFER_FMT_SET_FBITS(0))
...
#define AI_BUFFER_FMT_IS_Q(fmt_)                                                                   \
    ((AI_BUFFER_FMT_TYPE_Q == AI_BUFFER_FMT_GET_TYPE(fmt_)) && (1 == AI_BUFFER_FMT_GET_SIGN(fmt_)))
...
#define AI_BUFFER_FMT_IS_UQ(fmt_)                                                                  \
    ((AI_BUFFER_FMT_TYPE_Q == AI_BUFFER_FMT_GET_TYPE(fmt_)) && (0 == AI_BUFFER_FMT_GET_SIGN(fmt_)))
...
#define AI_ARRAY_FMT_SET_Q(bits_, fbits_) AI_ARRAY_FMT_Q(bits_, fbits_)
...
#define AI_ARRAY_FMT_SET_UQ(bits_, fbits_) AI_ARRAY_FMT_UQ(bits_, fbits_)
...
#define AI_ARRAY_FMT_SET_SQ(bits_, fbits_) AI_ARRAY_FMT_SET_Q(bits_, fbits_)

AI_DEPRECATED
/* Alias for AI_ARRAY_FMT_IS_Q */
#define AI_ARRAY_FMT_IS_SQ(fmt_) AI_ARRAY_FMT_IS_Q(fmt_)

/*! ai_array section **********************************************************/
#define AI_ARRAY_FMT_ENTRY(name_)                 AI_CONCAT(AI_ARRAY_FORMAT_, name_)

#define AI_ARRAY_FMT_NAME(fmt_)                   ai_array_fmt_name(fmt_)

#define AI_ARRAY_FMT_VALID(fmt_)                  ai_array_fmt_valid(fmt_)

#define AI_ARRAY_FMT_EXPORTED(fmt_)               ai_array_fmt_exported(fmt_)

#define AI_ARRAY_FMT_GET_FORMATS(formats_)        ai_array_fmt_get_formats(formats_)

#define AI_ARRAY_TO_BUFFER_FMT(fmt_)              ai_array_to_buffer_fmt(fmt_)

#define AI_ARRAY_GET_BYTE_SIZE(fmt_, count_)      ai_array_get_byte_size(fmt_, count_)

#define AI_ARRAY_GET_DATA_BYTE_SIZE(fmt_, count_) ai_array_get_data_byte_size(fmt_, count_)

#define AI_ARRAY_GET_ELEMS_FROM_SIZE(fmt_, size_) ai_array_get_elems_from_size(fmt_, size_)
 ...
 */
typedef enum {
#define FMT_ENTRY(exp_, name_, type_id_, sign_bit_, complex_bit_, pmask_, bits_, fbits_,           \
                  ldiv_bits_)                                                                      \
    AI_ARRAY_FMT_ENTRY(name_) =                                                                    \
        (AI_FMT_SET_COMPLEX(complex_bit_) | AI_FMT_SET_SIGN(sign_bit_) | AI_FMT_SET_BITS(bits_) |  \
         AI_FMT_SET_FBITS(fbits_) | AI_FMT_SET_PMASK(pmask_) | AI_FMT_SET_TYPE(type_id_) |         \
...
#define AI_BUFFER_FMT_ENTRY(name_)          AI_CONCAT(AI_BUFFER_FORMAT_, name_)

#define AI_BUFFER_FMT_NAME(type_)           ai_buffer_fmt_name(type_)

#define AI_BUFFER_FMT_VALID(type_)          ai_buffer_fmt_valid(type_)

#define AI_BUFFER_FMT_GET_FORMATS(formats_) ai_buffer_fmt_get_formats(formats_)

#define AI_BUFFER_TO_ARRAY_FMT(fmt_)        ai_buffer_to_array_fmt(fmt_)

#define AI_BUFFER_GET_BITS_SIZE(fmt)        AI_ARRAY_GET_BITS_SIZE(AI_BUFFER_TO_ARRAY_FMT(fmt))
 ...
 */
AI_INTERNAL_API
ai_bool ai_buffer_fmt_valid(const ai_buffer_format type);
 ...
 */
AI_INTERNAL_API
ai_buffer_format ai_array_to_buffer_fmt(const ai_array_format fmt);
 ...
 */
AI_INTERNAL_API
ai_array_format ai_buffer_to_array_fmt(const ai_buffer_format fmt);
 ...
 */
AI_INTERNAL_API
ai_size ai_array_get_byte_size(const ai_array_format fmt, const ai_size count);
 ...
 */
AI_INTERNAL_API
ai_size ai_array_get_data_byte_size(const ai_array_format fmt, const ai_size count);
 ...
 */
AI_INTERNAL_API
ai_size ai_array_get_elems_from_size(const ai_array_format fmt, const ai_size byte_size);
