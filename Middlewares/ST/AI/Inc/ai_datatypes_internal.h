...
#define AI_STORAGE_KLASS_TYPE(s_)        ((s_)->type)

#define AI_STORAGE_KLASS_SIZE(s_)        ((s_)->size)

#define AI_STORAGE_KLASS_DATA(s_, type_) ((type_ *)((s_)->data))
...
#define AI_STORAGE_KLASS_COPY(dst_, dst_type_, src_, src_type_)                                    \
    {                                                                                              \
        ...
        for (ai_size i = 0; i < AI_STORAGE_KLASS_SIZE(dst_); i++) {                                \
            AI_STORAGE_KLASS_DATA(dst_, dst_type_)[i] = AI_STORAGE_KLASS_DATA(src_, src_type_)[i]; \
        }                                                                                          \
    }
...
/*!  AI_SHAPES SECTION                     ************************************/
#define AI_SHAPE_2D_H(shape_)       AI_SHAPE_ELEM(shape_, AI_SHAPE_2D_HEIGHT)

#define AI_SHAPE_2D_W(shape_)       AI_SHAPE_ELEM(shape_, AI_SHAPE_2D_WIDTH)

#define AI_SHAPE_ELEM(shape_, pos_) AI_STORAGE_KLASS_DATA(shape_, ai_shape_dimension)[pos_]
...
#define AI_SHAPE_SET_ELEM(shape_, pos_, val_)                                                      \
    if ((pos_) < AI_SHAPE_SIZE(shape_)) {                                                          \
        AI_SHAPE_ELEM(shape_, pos_) = (val_);                                                      \
    }

#define AI_SHAPE_TYPE(shape_) AI_STORAGE_KLASS_TYPE(shape_)

#define AI_SHAPE_SIZE(shape_) AI_STORAGE_KLASS_SIZE(shape_)
...
#define AI_SHAPE_D(shape_)                                                                         \
    ((AI_SHAPE_SIZE((shape_)) > AI_SHAPE_DEPTH) ? AI_SHAPE_ELEM((shape_), AI_SHAPE_DEPTH) : 1)
#define AI_SHAPE_E(shape_)                                                                         \
    ((AI_SHAPE_SIZE((shape_)) > AI_SHAPE_EXTENSION) ? AI_SHAPE_ELEM((shape_), AI_SHAPE_EXTENSION)  \
                                                    : 1)
...
/*!  AI_STRIDES SECTION                     ***********************************/
#define AI_STRIDE_2D_H(stride_)       AI_STRIDE_ELEM((stride_), AI_SHAPE_2D_HEIGHT)

#define AI_STRIDE_2D_W(stride_)       AI_STRIDE_ELEM((stride_), AI_SHAPE_2D_WIDTH)

#define AI_STRIDE_ELEM(stride_, pos_) AI_STORAGE_KLASS_DATA(stride_, ai_stride_dimension)[pos_]
...
#define AI_STRIDE_SET_ELEM(stride_, pos_, val_)                                                    \
    if ((pos_) < AI_STRIDE_SIZE(stride_))                                                          \
        AI_STRIDE_ELEM(stride_, pos_) = (val_);

#define AI_STRIDE_TYPE(stride_) AI_STORAGE_KLASS_TYPE(stride_)

#define AI_STRIDE_SIZE(stride_) AI_STORAGE_KLASS_SIZE(stride_)
...
#define AI_STRIDE_D(stride)                                                                        \
    ((AI_STRIDE_SIZE((stride)) >= 5) ? AI_STRIDE_ELEM((stride), AI_SHAPE_DEPTH) : 0)
#define AI_STRIDE_E(stride)                                                                        \
    ((AI_STRIDE_SIZE((stride)) == 6) ? AI_STRIDE_ELEM((stride), AI_SHAPE_EXTENSION) : 0)
...
#define AI_STRIDE_SET_D(stride, val)                                                               \
    if (AI_STRIDE_SIZE((stride)) >= 5)                                                             \
    AI_STRIDE_SET_ELEM((stride), AI_SHAPE_DEPTH, val)
#define AI_STRIDE_SET_E(stride, val)                                                               \
    if (AI_STRIDE_SIZE((stride)) == 6)                                                             \
    AI_STRIDE_SET_ELEM((stride), AI_SHAPE_EXTENSION, val)

/*!  AI_TENSORS SECTION                     ***********************************/
#define AI_TENSOR_KLASS(tensor_)     ((tensor_) ? (tensor_)->klass : NULL)

#define AI_TENSOR_SHAPE(tensor_)     (&((tensor_)->shape))

#define AI_TENSOR_STRIDE(tensor_)    (&((tensor_)->stride))

#define AI_TENSOR_INFO(tensor_)      (&((tensor_)->info))

#define AI_TENSOR_ARRAY(tensor_)     ((tensor_) ? (tensor_)->data : NULL)

#define AI_TENSOR_ID(tensor_)        ((tensor_) ? AI_TENSOR_INFO(tensor_)->id : 0)

#define AI_TENSOR_FLAGS(tensor_)     ((tensor_) ? AI_TENSOR_INFO(tensor_)->flags : 0)

#define AI_TENSOR_DATA_SIZE(tensor_) ((tensor_) ? AI_TENSOR_INFO(tensor_)->data_size : 0)
...
#define AI_OFFSET_IN_CH(in_ch, stride) ((ai_ptr_offset)(in_ch)*AI_STRIDE_IN_CH(stride))
...
#define AI_OFFSET_5D(y, x, d, e, ch, stride)                                                       \
    (AI_OFFSET_H((y), (stride)) + AI_OFFSET_W((x), (stride)) + AI_OFFSET_D((d), (stride)) +        \
     AI_OFFSET_E((e), (stride)) + AI_OFFSET_CH((ch), (stride)))

#define AI_OFFSET(y, x, ch, z, stride)                                                             \
    (AI_OFFSET_H((y), (stride)) + AI_OFFSET_W((x), (stride)) + AI_OFFSET_CH((ch), (stride)) +      \
     ((AI_STRIDE_SIZE((stride)) == 4) ? AI_OFFSET_IN_CH((z), (stride))                             \
                                      : AI_OFFSET_D((z), (stride))))
...
#define AI_TENSOR_SIZE(tensor_)       get_tensor_size(tensor_, true)

#define AI_TENSOR_SIZE_UNPAD(tensor_) get_tensor_size(tensor_, false)

#define AI_TENSOR_BYTE_SIZE(tensor_)  get_tensor_byte_size(tensor_)
...
#define AI_INTQ_INFO_LIST_SCALE_ARRAY(list_, type_)                                                \
    (((list_) && (list_)->info) ? ((type_ *)((list_)->info->scale)) : NULL)

#define AI_INTQ_INFO_LIST_ZEROPOINT_ARRAY(list_, type_)                                            \
    (((list_) && (list_)->info) ? ((type_ *)((list_)->info->zeropoint)) : NULL)

#define AI_KLASS_GET_INTQ_INFO_LIST(tensor_) ((ai_intq_info_list *)((tensor_)->klass))
 ...
 */
AI_DECLARE_STATIC
ai_bool ai_shape_is_scalar(const ai_shape * shape0) {
    ai_size dim = AI_SHAPE_SIZE(shape0);
    while (dim > 0) {
        dim--;
