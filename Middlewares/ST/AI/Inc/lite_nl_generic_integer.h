...
/**
 * @brief forward lite function for a s8 softmax non-linearity where the softmax is applied per
 * channel.
 ...
 */
LITE_API_ENTRY
void forward_lite_nl_softmax_is8os8(ai_i8 * out_ptr, const ai_i8 * in_ptr, const ai_size in_size,
                                    const ai_size inner_loop_cnt, const ai_size axis_elem,
                                    const ai_i32 mult, const ai_i32 shift, const ai_i32 min_diff,
                                    ai_i32 * scratch);

/**
 * @brief forward lite function for a u8 softmax non-linearity where the softmax is applied per
 * channel.
 * @ingroup lite_nl_generic_integer
 * @param output The pointer to output buffer (s8).
 * @param input The pointer to input buffer (s8).
 ...
 */
LITE_API_ENTRY
void forward_lite_nl_softmax_iu8ou8(ai_u8 * out_ptr, const ai_u8 * in_ptr, const ai_size in_size,
                                    const ai_size inner_loop_cnt, const ai_size axis_elem,
                                    const ai_i32 mult, const ai_i32 shift, const ai_i32 min_diff,
                                    ai_i32 * scratch);
