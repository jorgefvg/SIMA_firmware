...
/*****************************************************************************/
#define CORE_ADDRESS_RANGE_INIT(start_, end_) core_address_range_init(start_, end_)

#define CORE_GET_BUFFER_META_INFO(meta_info_, tensor_ptr_)                                         \
    core_get_buffer_meta_info(meta_info_, tensor_ptr_)

#define CORE_ADDRESS_RANGE_END(range_)       ((ai_ptr)(((range_)->start) + ((range_)->size)))

#define CORE_ADDRESS_RANGE_OVERLAP(overlap_) (((overlap_)->start) && (((overlap_)->size) > 0))
...
#define CORE_MEMORY_OVERLAP_INIT(partial_, range_, chain_id_, tensor_id_)                          \
    {                                                                                              \
        .partial = (partial_), .range = AI_PACK(range_), .chain_id = (chain_id_),                  \
...
/*****************************************************************************/
#define AI_TENSOR_HAS_INTQ_INFO             AI_BUFFER_META_HAS_INTQ_INFO

#define CORE_TENSOR_GET_SHAPE_SIZE(tensor_) ai_shape_get_size(AI_TENSOR_SHAPE(tensor_))
...
#define CORE_ASSERT_SHAPE_MATCH(x, y)                                                              \
    do {                                                                                           \
        ...
        AI_ASSERT(AI_SHAPE_IN_CH(y) == 1 || AI_SHAPE_IN_CH(x) == 1 ||                              \
                  AI_SHAPE_IN_CH(y) == AI_SHAPE_IN_CH(x))                                          \
    } while (0);

#define AI_TENSOR_ARRAY_BYTE_SIZE(t_)     AI_ARRAY_OBJ_BYTE_SIZE(AI_ARRAY_OBJ(t_->data))

#define AI_TENSOR_ARRAY_GET_DATA_ADDR(t_) AI_HANDLE_PTR(AI_ARRAY_OBJ_DATA_START(t_->data, void))
...
#define AI_TENSOR_INTEGER_GET_SIZE(t_) ((t_->klass) ? (AI_KLASS_GET_INTQ_INFO_LIST(t_))->size : 0)
...
#define AI_TENSOR_FMT_GET_SIGN(t_)  AI_BUFFER_FMT_GET_SIGN(AI_ARRAY_OBJ(t_->data)->format)

#define AI_TENSOR_FMT_GET_BITS(t_)  AI_BUFFER_FMT_GET_BITS(AI_ARRAY_OBJ(t_->data)->format)

#define AI_TENSOR_FMT_GET_FBITS(t_) AI_BUFFER_FMT_GET_FBITS(AI_ARRAY_OBJ(t_->data)->format)

#define AI_TENSOR_FMT_GET_TYPE(t_)  AI_BUFFER_FMT_GET_TYPE(AI_ARRAY_OBJ(t_->data)->format)

#define AI_TENSOR_GET_FMT(t_)       AI_FMT_OBJ(AI_ARRAY_OBJ(t_->data)->format)

/*****************************************************************************/
/** Network Buffers Handlers                                                 **/
/*****************************************************************************/
#define AI_FOR_EACH_BUFFER_ARRAY_ITEM(buffer_ptr_, buffer_array_ptr_, start_pos_, end_pos_)        \
    ai_buffer * buffer_ptr_ = AI_BUFFER_ARRAY_ITEM(                                                \
        buffer_array_ptr_, CORE_OFFSET(end_pos_, AI_BUFFER_ARRAY_SIZE(buffer_array_ptr_)));        \
    for (; buffer_ptr_ && AI_BUFFER_ARRAY_SIZE(buffer_array_ptr_) &&                               \
           (buffer_ptr_ >= AI_BUFFER_ARRAY_ITEM(                                                   \
                               buffer_array_ptr_,                                                  \
                               CORE_OFFSET(start_pos_, AI_BUFFER_ARRAY_SIZE(buffer_array_ptr_)))); \
         buffer_ptr_--)

/*****************************************************************************/
/** Network Arrays Handlers                                                 **/
/*****************************************************************************/
#define AI_ARRAY_OBJ_FMT(array_)     AI_FMT_OBJ(AI_ARRAY_OBJ(array_)->format)

#define AI_ARRAY_OBJ_FMT_GET(array_) AI_FMT_GET(AI_ARRAY_OBJ_FMT(array_))

#define AI_ARRAY_OBJ_SIZE(array_)    (AI_ARRAY_OBJ(array_)->size)

#define AI_ARRAY_OBJ_BYTE_SIZE(array_)                                                             \
    AI_SIZE(AI_ARRAY_GET_BYTE_SIZE(AI_ARRAY_OBJ_FMT(array_), AI_ARRAY_OBJ_SIZE(array_)))

#define AI_ARRAY_OBJ_DATA_SIZE(array_)                                                             \
    AI_ARRAY_GET_DATA_BYTE_SIZE(AI_ARRAY_OBJ_FMT(array_), AI_ARRAY_OBJ_SIZE(array_))

#define AI_ARRAY_OBJ_DATA(array_, type_)       AI_CAST(type_ *, AI_ARRAY_OBJ(array_)->data)

#define AI_ARRAY_OBJ_DATA_START(array_, type_) AI_CAST(type_ *, AI_ARRAY_OBJ(array_)->data_start)

#define AI_ARRAY_OBJ_ELEM(array_, type_, pos_) AI_ARRAY_OBJ_DATA(array_, type_)[(pos_)]

/*****************************************************************************/
/** Network Tensors Chains / Lists Handlers                                 **/
/*****************************************************************************/
#define SET_TENSOR_IN(chain_, pos_)  (GET_TENSOR_LIST_IN(chain_)->tensor[(pos_)])

#define SET_TENSOR_OUT(chain_, pos_) (GET_TENSOR_LIST_OUT(chain_)->tensor[(pos_)])
...
/*****************************************************************************/
#define AI_BITS_TO_BYTES(bits_)  (((bits_) + 0x7) >> 3)

#define AI_BYTES_TO_BITS(bytes_) ((bytes_) << 3)
...
#define AI_NODE_IS_LAST(node_)                                                                     \
    ((AI_NODE_OBJ(node_) == AI_NODE_OBJ(node_)->next) || (AI_NODE_OBJ(node_)->next == NULL))
...
/*****************************************************************************/
AI_DECLARE_STATIC
ai_address_range core_address_range_init(const ai_handle start, const ai_handle end) {
    ai_address_range r;

    r.start = (ai_ptr)((start < end) ? start : end);
    r.size = (ai_size)((start < end) ? ((ai_uptr)end - (ai_uptr)start)
                                     : ((ai_uptr)start - (ai_uptr)end));
    return r;
}

AI_DECLARE_STATIC
ai_buffer_meta_info * core_get_buffer_meta_info(ai_buffer_meta_info * meta, const ai_tensor * t) {
    if (!meta)
        return NULL;
    AI_ASSERT(t && t->data)
    ai_bool ok;
