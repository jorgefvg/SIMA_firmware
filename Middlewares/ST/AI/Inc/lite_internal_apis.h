...
#define LITE_NL_ENTRY(nl_id_, nl_name_, nl_op_, nl_op_args_)                                       \
     ...
     */                                                                                            \
    LITE_API_ENTRY                                                                                 \
    void forward_lite_nl_##nl_name_##_if32of32(ai_handle out_ptr, const ai_handle in_ptr,          \
 ...
 */
LITE_API_ENTRY
void forward_lite_nl_softmax_if32of32(ai_handle out_ptr, const ai_handle in_ptr,
                                      const ai_size in_size, const ai_size inner_loop_cnt,
                                      const ai_size axis_elem);

/**
 * @brief lite function for a float softmax zero channel non-linearity where the softmax is applied
 * per channel.
 * @ingroup lite_nl_generic_float
 * @param output The pointer to output buffer.
 * @param input The pointer to input buffer.
 ...
 */
LITE_API_ENTRY
void forward_lite_nl_softmax_zero_channel_if32of32(ai_handle out_ptr, const ai_handle in_ptr,
                                                   const ai_i32 in_size, const ai_size ch_size,
                                                   const ai_i32 in_ch_step,
                                                   const ai_i32 out_ch_step);
 ...
 */
typedef void (*func_nl_lite)(ai_handle out_ptr, const ai_handle in_ptr, const ai_i32 in_size,
 ...
 */
LITE_API_ENTRY
void forward_lite_nl_gelu_if32of32(ai_handle out_ptr, const ai_handle in_ptr, const ai_i32 in_size,
                                   const ai_handle params);
