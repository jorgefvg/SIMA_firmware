...
#define AI_PLATFORM_API_VERSION                                                                    \
    AI_VERSION(AI_PLATFORM_API_MAJOR, AI_PLATFORM_API_MINOR, AI_PLATFORM_API_MICRO)
...
#define AI_TOOLS_API_VERSION                                                                       \
    AI_VERSION(AI_TOOLS_API_VERSION_MAJOR, AI_TOOLS_API_VERSION_MINOR, AI_TOOLS_API_VERSION_MICRO)

#define AI_TOOLS_API_VERSION_1_3 AI_VERSION(1, 3, 0)

#define AI_TOOLS_API_VERSION_1_4 AI_VERSION(1, 4, 0)

#define AI_TOOLS_API_VERSION_1_5 AI_VERSION(1, 5, 0)
...
#define AI_CUSTOM_SIZE(type)                                                                       \
    (ai_custom_type_signature)((AI_IS_UNSIGNED(type)) ? (0x80 | (sizeof(type) & 0x7f))             \
                                                      : (sizeof(type) & 0x7f))
...
#define AI_NETWORK_PARAMS_INIT(params_, activations_)                                              \
    { .params = params_, .activations = activations_ }

#define AI_NETWORK_BUFFERS_INIT(weights_buffers_, activations_buffers_)                            \
    {                                                                                              \
        .map_signature = AI_MAGIC_SIGNATURE, .map_weights = AI_PACK(weights_buffers_),             \
...
#define AI_PBITS_MASK                      (0x1F)

#define AI_PBITS_SHIFTS                    (5)

#define AI_PBITS_PADDED_BYTES_COUNT(bits_) (((ai_u32)(bits_) + 7) >> 3)

#define AI_PBITS_PADDED_WORDS_COUNT(bits_) (((ai_size)(bits_) + AI_PBITS_MASK) >> AI_PBITS_SHIFTS)

#define AI_PBITS_GET_WORD(word_ptr_, bits_)                                                        \
    (((ai_pbits *)(word_ptr_)) + ((bits_) >> AI_PBITS_SHIFTS))

#define AI_PAD_CHANNELS(format_, channels_)                                                        \
    ((AI_BUFFER_FMT_GET_BITS(format_) == 1)                                                        \
         ? (AI_PBITS_PADDED_WORDS_COUNT(channels_) << AI_PBITS_SHIFTS)                             \
         : (channels_))
...
#define AI_INTQ_INFO_LIST(list_)       ((list_)->info)

#define AI_INTQ_INFO_LIST_FLAGS(list_) ((list_) ? (list_)->flags : 0)

#define AI_INTQ_INFO_LIST_SIZE(list_)  ((list_) ? (list_)->size : 0)

#define AI_HAS_INTQ_INFO_LIST(list_)   ((list_) ? (((list_)->info) && ((list_)->size > 0)) : false)

#define AI_INTQ_INFO_LIST_SCALE(list_, type_, pos_)                                                \
    (((list_) && (list_)->info && ((pos_) < (list_)->size))                                        \
         ? ((type_ *)((list_)->info->scale))[(pos_)]                                               \
         : 1.0f)

#define AI_INTQ_INFO_LIST_ZEROPOINT(list_, type_, pos_)                                            \
    (((list_) && (list_)->info && ((pos_) < (list_)->size))                                        \
         ? ((type_ *)((list_)->info->zeropoint))[(pos_)]                                           \
         : 0)
...
#define AI_BUFFER_FMT_PACK(value_, mask_, bits_) (((value_) & (mask_)) << (bits_))

#define AI_BUFFER_FMT_UNPACK(fmt_, mask_, bits_) ((AI_BUFFER_FMT_OBJ(fmt_) >> (bits_)) & (mask_))

#define AI_BUFFER_FMT_OBJ(fmt_)                  ((ai_buffer_format)(fmt_))

#define AI_BUFFER_FMT_GET_FLOAT(fmt_)            AI_BUFFER_FMT_UNPACK(fmt_, 0x1, 24)

#define AI_BUFFER_FMT_GET_SIGN(fmt_)             AI_BUFFER_FMT_UNPACK(fmt_, 0x1, 23)

#define AI_BUFFER_FMT_GET_TYPE(fmt_)             AI_BUFFER_FMT_UNPACK(fmt_, 0xF, 17)

#define AI_BUFFER_FMT_GET_BITS(fmt_)             AI_BUFFER_FMT_UNPACK(fmt_, 0x7F, 7)

#define AI_BUFFER_FMT_SET_BITS(bits_)            AI_BUFFER_FMT_PACK((bits_), 0x7F, 7)

#define AI_BUFFER_FMT_GET_FBITS(fmt_)            ((ai_i8)AI_BUFFER_FMT_UNPACK(fmt_, 0x7F, 0) - 64)

#define AI_BUFFER_FMT_SET_FBITS(fbits_)          AI_BUFFER_FMT_PACK((fbits_) + 64, 0x7F, 0)

#define AI_BUFFER_FMT_SET(type_id_, sign_bit_, float_bit_, bits_, fbits_)                          \
    AI_BUFFER_FMT_OBJ(AI_BUFFER_FMT_PACK(0, 0x1, 24) | AI_BUFFER_FMT_PACK(sign_bit_, 0x1, 23) |    \
                      AI_BUFFER_FMT_PACK(0, 0x3, 21) | AI_BUFFER_FMT_PACK(type_id_, 0xF, 17) |     \
                      AI_BUFFER_FMT_PACK(0, 0x7, 14) | AI_BUFFER_FMT_SET_BITS(bits_) |             \
                      AI_BUFFER_FMT_SET_FBITS(fbits_))

#define AI_BUFFER_FMT_SET_COMPLEX(type_id_, sign_bit_, bits_, fbits_)                              \
    AI_BUFFER_FMT_OBJ(AI_BUFFER_FMT_PACK(1, 0x1, 24) | AI_BUFFER_FMT_PACK(sign_bit_, 0x1, 23) |    \
                      AI_BUFFER_FMT_PACK(0, 0x3, 21) | AI_BUFFER_FMT_PACK(type_id_, 0xF, 17) |     \
                      AI_BUFFER_FMT_PACK(0, 0x7, 14) | AI_BUFFER_FMT_SET_BITS(bits_) |             \
                      AI_BUFFER_FMT_SET_FBITS(fbits_))

#define AI_BUFFER_FMT_SAME(fmt1_, fmt2_) (AI_BUFFER_FMT_GET(fmt1_) == AI_BUFFER_FMT_GET(fmt2_))

#define AI_BUFFER_FMT_GET(fmt_)          (AI_BUFFER_FMT_OBJ(fmt_) & AI_BUFFER_FMT_MASK)

#define AI_BUFFER_FORMAT(buf_)           AI_BUFFER_FMT_GET((buf_)->format)
...
AI_DEPRECATED
#define AI_BUFFER_WIDTH(buf_) ((buf_)->shape.data[AI_SHAPE_WIDTH])

AI_DEPRECATED
#define AI_BUFFER_HEIGHT(buf_) ((buf_)->shape.data[AI_SHAPE_HEIGHT])

AI_DEPRECATED
#define AI_BUFFER_CHANNELS(buf_) ((buf_)->shape.data[AI_SHAPE_CHANNEL])

AI_DEPRECATED
#define AI_BUFFER_N_BATCHES(buf_)   ((buf_)->shape.data[AI_SHAPE_BATCH])

#define AI_BUFFER_DATA(buf_, type_) ((type_ *)((buf_)->data))

#define AI_BUFFER_META_INFO(buf_)   ((buf_)->meta_info)

#define AI_BUFFER_META_INFO_INTQ(meta_)                                                            \
    ((meta_) && ((meta_)->flags & AI_BUFFER_META_HAS_INTQ_INFO)) ? ((meta_)->intq_info) : NULL

#define AI_BUFFER_META_INFO_INTQ_GET_SIZE(meta_)                                                   \
    ((AI_BUFFER_META_INFO_INTQ(meta_)) ? AI_INTQ_INFO_LIST_SIZE(AI_BUFFER_META_INFO_INTQ(meta_))   \
                                       : 0)
...
#define AI_BUFFER_META_INFO_INTQ_GET_ZEROPOINT(meta_, pos_)                                        \
    ((AI_BUFFER_META_INFO_INTQ(meta_))                                                             \
         ? ((AI_INTQ_INFO_LIST_FLAGS(AI_BUFFER_META_INFO_INTQ(meta_)) &                            \
             AI_BUFFER_META_FLAG_ZEROPOINT_U8)                                                     \
...
#define AI_BUFFER_META_INFO_INIT(flags_, intq_info_)                                               \
    { .flags = (flags_), .intq_info = AI_PACK(intq_info_) }

#define AI_BUFFER_SIZE(buf_)              ai_buffer_get_size(buf_, true)

#define AI_BUFFER_SIZE_UNPAD(buf_)        ai_buffer_get_size(buf_, false)

#define AI_BUFFER_BYTE_SIZE(count_, fmt_) ai_buffer_get_byte_size(count_, fmt_)

#define AI_BUFFER_FLAGS(buf_)             ((buf_) ? (buf_)->flags : 0x0)

#define AI_BUFFER_SHAPE_INIT(type_, size_, ...)                                                    \
    {                                                                                              \
        .type = (type_), .size = (size_), .data = (ai_shape_dimension[]) { __VA_ARGS__ }           \
    }

#define AI_BUFFER_SHAPE_INIT_FROM_ARRAY(type_, size_, array_ptr_)                                  \
    { .type = (type_), .size = (size_), .data = (ai_shape_dimension *)(array_ptr_) }

#define AI_BUFFER_SHAPE_SIZE(buf_) ((buf_) ? (buf_)->shape.size : 0)

#define AI_BUFFER_SHAPE_TYPE(buf_) ((buf_) ? (buf_)->shape.type : 0)
...
#define AI_BUFFER_SHAPE_ELEM(buf_, pos_) (buf_)->shape.data[pos_]
...
#define AI_BUFFER_OBJ_INIT(format_, h_, w_, ch_, n_batches_, data_)                                \
    {                                                                                              \
        .format = (ai_buffer_format)(format_), .data = (ai_handle)(data_), .meta_info = NULL,      \
        .flags = AI_FLAG_NONE, .size = (h_) * (w_)*AI_PAD_CHANNELS(format_, ch_),                  \
...
#define AI_BUFFER_INIT(flags_, format_, shape_, size_, meta_info_, data_)                          \
    {                                                                                              \
        .format = (ai_buffer_format)(format_), .data = (ai_handle)(data_),                         \
        .meta_info = (meta_info_), .flags = (flags_), .size = (size_), .shape = AI_PACK(shape_)    \
    }

/* 7.1 new macro API */
#define AI_BUFFER_INIT_STATIC(type_, flags_, format_, shape_, size_, meta_info_, ...)              \
    {                                                                                              \
        .format = (ai_buffer_format)(format_), .data = (ai_handle)((type_[]){__VA_ARGS__}),        \
        .meta_info = (meta_info_), .flags = (flags_), .size = (size_), .shape = AI_PACK(shape_)    \
    }
...
#define AI_BUFFER_ARRAY_OBJ_INIT(flags_, size_, buffer_array_)                                     \
    { .flags = (ai_u16)(flags_), .size = (ai_u16)(size_), .buffer = (ai_buffer *)(buffer_array_) }

#define AI_BUFFER_ARRAY_OBJ_INIT_STATIC(flags_, size_, ...)                                        \
    {                                                                                              \
        .flags = (ai_u16)(flags_), .size = (ai_u16)(size_),                                        \
        .buffer = (ai_buffer *)((ai_buffer[]){__VA_ARGS__})                                        \
    }

#define AI_BUFFER_ARRAY_SANE(buf_array_) ai_buffer_array_sane(buf_array_)
...
#define AI_ERROR_INIT(type_, code_)                                                                \
    { .type = AI_ERROR_##type_, .code = AI_ERROR_CODE_##code_ }
...
#define AI_VERSION(major_, minor_, micro_) (((major_) << 24) | ((minor_) << 16) | ((micro_) << 8))
 ...
 */
typedef enum { AI_SCATTER_ND_NONE = 0x0, AI_SCATTER_ND_OPERATION } ai_scatter_nd_reduction;
 ...
 */
AI_API_ENTRY
ai_bool ai_buffer_array_item_set_address(ai_buffer_array * barray, const ai_u32 pos,
                                         ai_handle address);
